...
function LekMapgenFormatBench6SummaryLine(startDb)
	...
	local sa = tostring(_lek_bench_spacing_salt_adj_n or "?");
	...
	local regenWhy = tostring(_lek_global_six_request_map_regen_reason or "na");
	...
	return "### LekBench6 runId=" .. rid
		...
		.. " cs_discarded=" .. csd
		.. " hex_ok=" .. tostring(_lek_bench_hex_ok or "?")
		.. " hex_rot=" .. tostring(_lek_bench_hex_rot or "?")
		.. " hex_ringR=" .. tostring(_lek_bench_hex_ringR or "?")
		.. " feas_bn=" .. tostring(_lek_bench_feas_bn or "?")
		.. " feas_xmin=" .. tostring(_lek_bench_feas_xmin or "?")
		.. " feas_h2max=" .. tostring(_lek_bench_feas_h2max or "?")
		.. " feas_margmin=" .. tostring(_lek_bench_feas_margmin or "?")
		.. " feas_k=" .. tostring(_lek_bench_feas_k or "?");
end
...
function LekMapgenDiagAt(minLevel, lineOrLines)
	if not LekMapgenLogAtLeast(minLevel) then
		return;
	end
	LekMapgenDiagLogAppend(lineOrLines);
end
...
--
-- Lekmap naming (capital / rings near start): regional luxury bundle and
-- second-lux cascade are luxuries only. Strategics (iron, horses, …) use
-- other passes and are not part of the "capital goodies" counter. Capital
-- goodies = fish plus LekResourceIsBonus tiles (wheat, sheep, stone, …) in
-- LekCountCapitalGoodiesBudgetNearStart; coastal fish uses goodies_cap=3 for
-- that combined budget. AttemptToPlaceBonusResourceAtPlot uses a separate
-- per-call maxCapitalGoodiesLeft counter for legacy ring placement.
...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		LekTestCopperOnAllWater = AssignStartingPlots.LekTestCopperOnAllWater,
		...
		LekTryStripOneBonusFromRingsNearStart = AssignStartingPlots.LekTryStripOneBonusFromRingsNearStart,
		...
		LekCountCapitalGoodiesBudgetNearStart = AssignStartingPlots.LekCountCapitalGoodiesBudgetNearStart,
		LekCountGranaryBudgetNearStart = AssignStartingPlots.LekCountCapitalGoodiesBudgetNearStart,
		LekSortPlotIndicesMainlandFirst = AssignStartingPlots.LekSortPlotIndicesMainlandFirst,
		LekCountWorkableLandTilesInRadiusFromXY = AssignStartingPlots.LekCountWorkableLandTilesInRadiusFromXY,
		LekCoastalRegionalTilePassesWorkableLandRule = AssignStartingPlots.LekCoastalRegionalTilePassesWorkableLandRule,
		LekIsPlotWithinDistanceOfAnyCityState = AssignStartingPlots.LekIsPlotWithinDistanceOfAnyCityState,
		...
		LekSanitizeSeaLuxuriesOnLakeTiles = AssignStartingPlots.LekSanitizeSeaLuxuriesOnLakeTiles,
		...
		LekEnforceBalancedCoastalExactTwoAfterBalanceAndAssign = AssignStartingPlots.LekEnforceBalancedCoastalExactTwoAfterBalanceAndAssign,
		LekCountMajorPlayerStartsAdjacentToSaltWater = AssignStartingPlots.LekCountMajorPlayerStartsAdjacentToSaltWater,
		...
		LekEnumeratePlotIndicesOnHexRing = AssignStartingPlots.LekEnumeratePlotIndicesOnHexRing,
		LekMarblePerfumeCoerceHillAtPlot = AssignStartingPlots.LekMarblePerfumeCoerceHillAtPlot,
		LekEnsureHillForMarblePerfumePlot = AssignStartingPlots.LekEnsureHillForMarblePerfumePlot,
		LekBuildMarblePerfumeStartPlotListRing3Hill = AssignStartingPlots.LekBuildMarblePerfumeStartPlotListRing3Hill,
		...
		coast_list_panagaea = {},
		lake_coast_fish_list = {},
		plains_hills_open_list = {},
		marble_list = {},
		extra_deer_list = {},
		desert_wheat_list = {},
		banana_list = {},
...
function AssignStartingPlots:PlaceImpactAndRipples(x, y)
	...
	local ripple_decider = LekMapGetCustomOption(6);
...
function AssignStartingPlots:LekGlobalSix_LogRippleOrderedSampleDryRun()
	...
	if failPick then
		restoreAll();
		LekPlacementProbeAt(2, "### LekGlobalSix rippleOrderDryRun runId=" .. rid ..
...
function AssignStartingPlots:LekGlobalSix_NoCoastInlandEnforced()
	...
	local ok, dsb = pcall(function()
		return Game.GetCustomOption("GAMEOPTION_DISABLE_START_BIAS");
	end);
	if ok and dsb == 1 then
		return false;
	end
	return true;
end
...
function AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, ax, ay, bx, by, baseMin)
	if type(baseMin) ~= "number" or baseMin < 2 then
		return baseMin;
	end
	local mA = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, ax, ay);
	local mB = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, bx, by);
	local req = baseMin;
	if mA.alongOcean == true and mB.alongOcean == true and req < 11 then
		req = 11;
	end
	return req;
end
...
function AssignStartingPlots:LekGlobalSix_OK_Section2_d2()
	...
	local dMinPair = AssignStartingPlots.LekGlobalSix_MinPairwiseDistanceFromStartDistanceOption();
	if type(dMinPair) == "number" and dMinPair >= 2 then
		for i = 1, self.iNumCivs do
			local ti = self.startingPlots[i];
			if not ti or type(ti[1]) ~= "number" or type(ti[2]) ~= "number" then
				return false, "missing_i_pair=" .. tostring(i);
			end
			for j = i + 1, self.iNumCivs do
				local tj = self.startingPlots[j];
				if not tj or type(tj[1]) ~= "number" or type(tj[2]) ~= "number" then
					return false, "missing_j_pair=" .. tostring(j);
				end
				local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, ti[1], ti[2], tj[1], tj[2]);
				if d == nil then
					return false, "no_PlotDistance_pair";
				end
				local req = AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, ti[1], ti[2], tj[1], tj[2], dMinPair);
				if d < req then
					return false, string.format("pair_ij=%d,%d dist=%d minPair=%d opt6", i, j, d, req);
				end
			end
		end
	end
...
function AssignStartingPlots.LekGlobalSix_SecondNearestD2ForRegion_i(self, i)
	local ti = self.startingPlots[i];
	if not ti or type(ti[1]) ~= "number" or type(ti[2]) ~= "number" then
		return nil;
	end
	local dists = {};
	for j = 1, self.iNumCivs or 0 do
		if i ~= j then
			local tj = self.startingPlots[j];
			if not tj or type(tj[1]) ~= "number" or type(tj[2]) ~= "number" then
				return nil;
...
function AssignStartingPlots.LekGlobalSix_OK_Section2_MinPairSubclauseOnly(self)
	local dMinPair = AssignStartingPlots.LekGlobalSix_MinPairwiseDistanceFromStartDistanceOption();
	if type(dMinPair) ~= "number" or dMinPair < 2 then
		return true, "";
	end
	for i = 1, self.iNumCivs do
		local ti = self.startingPlots[i];
...
function AssignStartingPlots.LekGlobalSix_QualifiesMinimalS2Plus1NearMiss(self, s2cap)
	...
	local d2ByR = {};
	for i = 1, 6 do
		local d2 = AssignStartingPlots.LekGlobalSix_SecondNearestD2ForRegion_i(self, i);
		if d2 == nil then
			return false, nil;
		end
		d2ByR[i] = d2;
	...
	if maxD2 ~= s2cap + 1 or nStrict < 1 or nStrict > 2 then
		return false, { maxD2 = maxD2, nOverCap = nStrict, s2cap = s2cap };
	end
	local ok1, _ = AssignStartingPlots.LekGlobalSix_OK_Section1_CentreBand(self);
	if not ok1 then
		return false, nil;
	end
	local okP, _ = AssignStartingPlots.LekGlobalSix_OK_Section2_MinPairSubclauseOnly(self);
	if not okP then
		return false, nil;
	end
	local ok3, _ = AssignStartingPlots.LekGlobalSix_OK_Section3_InlandSalt(self);
	if not ok3 then
		return false, nil;
	end
	local ok5, _ = AssignStartingPlots.LekGlobalSix_OK_Section5_BalanceAndAssignFeasible(self);
	if not ok5 then
		return false, nil;
	end
	local ok6, _ = AssignStartingPlots.LekGlobalSix_OK_Section6_SiteQuality_PostPlacement(self);
	if not ok6 then
		return false, nil;
	end
	return true, { maxD2 = maxD2, nOverCap = nStrict, s2cap = s2cap, d2ByR = d2ByR };
end

function AssignStartingPlots.LekGlobalSix_TupleFailComboS5Heavy(failComboHist)
	if type(failComboHist) ~= "table" or not next(failComboHist) then
		return false;
	end
	local s2pure = failComboHist["s2"];
	if type(s2pure) ~= "number" then
		s2pure = 0;
	end
	local with5 = 0;
	for k, v in pairs(failComboHist) do
		if type(k) == "string" and type(v) == "number" and string.find(k, "s5", 1, true) then
			with5 = with5 + v;
		end
	end
	return with5 > s2pure;
...
function AssignStartingPlots.LekGlobalSix_TupleFailHistPairHeavy(failHist)
	if type(failHist) ~= "table" or not next(failHist) then
		return false;
	end
	local total = 0;
	local pairn = 0;
	for k, v in pairs(failHist) do
		if type(v) == "number" and v > 0 then
			total = total + v;
			if tostring(k) == "pair" then
				pairn = pairn + v;
			end
		end
	end
	...
	return (pairn / total) >= 0.85;
end

-- Backward-compatible alias while old option names may still exist in user configs/log scripts.
function AssignStartingPlots.LekGlobalSix_TupleFailHistS4Heavy(failHist)
	return AssignStartingPlots.LekGlobalSix_TupleFailHistPairHeavy(failHist);
end

function AssignStartingPlots.LekGlobalSix_TuplePhaseIsS2OnlyRelaxVsBase(ph, basePh)
	if type(ph) ~= "table" or type(basePh) ~= "table" then
		return false;
	end
	local bs1a = basePh.s1_min;
	local bs1b = basePh.s1_max;
	local bs2 = basePh.s2_max;
	local ps1a = ph.s1_min;
	local ps1b = ph.s1_max;
	local ps2 = ph.s2_max;
	if type(bs1a) ~= "number" then
		bs1a = LEK_G6_S1_D_MIN;
	end
	if type(bs1b) ~= "number" then
		bs1b = LEK_G6_S1_D_MAX;
	end
	if type(bs2) ~= "number" then
		bs2 = LEK_G6_S2_SECOND_NEAREST_MAX;
	end
	if type(ps1a) ~= "number" then
		ps1a = LEK_G6_S1_D_MIN;
	end
	if type(ps1b) ~= "number" then
		ps1b = LEK_G6_S1_D_MAX;
	end
	if type(ps2) ~= "number" then
		ps2 = LEK_G6_S2_SECOND_NEAREST_MAX;
	end
	return ps1a == bs1a and ps1b == bs1b and ps2 > bs2;
end

function AssignStartingPlots.LekGlobalSix_TupleShouldExtendBudgetPureS2(failHist, failComboHist)
	if type(failHist) ~= "table" then
		return false;
	end
	...
	return not AssignStartingPlots.LekGlobalSix_TupleFailComboS5Heavy(failComboHist or {});
end
...
function AssignStartingPlots.LekGlobalSix_CoastalCandidatePassesSaltWaterDiskPct(self, plotIndex)
	...
	local rad = self._lek_global_six_coastal_salt_water_disk_radius;
	if type(rad) ~= "number" or rad < 1 then
		rad = 3;
	end
	local iW, iH = Map.GetGridSize();
	local x = (plotIndex - 1) % iW;
	...
	return (nSalt * 100 <= pctMax * tot);
end

function AssignStartingPlots.LekGlobalSix_CoastalSaltDiskPctFilterActive(self)
	local pctMax = self._lek_global_six_coastal_disk3_max_salt_water_pct;
	return type(pctMax) == "number" and pctMax >= 0 and pctMax <= 100;
end
...
function AssignStartingPlots:LekGlobalSix_OK_Section4_TwoCoastalGeoCycle()
	...
	local cx = math.floor(iW / 2);
	local cy = math.floor(iH / 2);
	local function isCoastalForCycleRule(x, y)
		local m = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, x, y);
		return m.alongOcean == true;
	end
	local slots = {};
	for r = 1, 6 do
		local t = self.startingPlots[r];
		...
		local x, y = t[1], t[2];
		local isC = isCoastalForCycleRule(x, y);
	...
	if forceExactTwo then
		if nC ~= 2 then
			return false, "force2_need_exactly2_got=" .. tostring(nC) .. "_def=saltOcean";
		end
	else
		if nC ~= 2 then
			return true, "n/a_coastalCount=" .. tostring(nC) .. "_def=saltOcean";
		end
	end
...
function AssignStartingPlots:LekGlobalSix_MeasureBiasConditionsAtXY(x, y)
	...
	local plotIndex = y * iW + x + 1;
	local plot = Map.GetPlot(x, y);
	local alongOcean = (self.plotDataIsCoastal and self.plotDataIsCoastal[plotIndex] == true);
...
function AssignStartingPlots.LekGlobalSix_BuildEffectiveBiasPolicy(self)
	local policy = {
		coastal_required = {},
		target_coastal = nil,
	};
	if not self or (self.iNumCivs or 0) ~= 6 then
		return policy;
	end
	local pmeta = {};
	for loop = 1, 6 do
		local pid = self.player_ID_list[loop];
		local player = Players[pid];
		local row = player and GameInfo.Civilizations[player:GetCivilizationType()];
		local civType = row and row.Type;
		local natural = false;
		local placeFirst = false;
		if civType then
			natural = CivNeedsCoastalStart(civType) == true;
			placeFirst = CivNeedsPlaceFirstCoastalStart(civType) == true;
			if self.MixedBias and placeFirst then
				natural = false;
			end
		end
		pmeta[#pmeta + 1] = { pid = pid, natural = natural, placeFirst = placeFirst };
		if natural then
			policy.coastal_required[pid] = true;
		end
	end
	if self._lek_ignore_all_start_bias == true or self.ForceAllInlandPlayerSpawns == true then
		policy.target_coastal = 0;
		policy.coastal_required = {};
		return policy;
	end
	if self._lek_tuple_force_two_coastal_majors == true then
		policy.target_coastal = 2;
		local function countReq()
			local n = 0;
			for _, pm in ipairs(pmeta) do
				if policy.coastal_required[pm.pid] == true then
					n = n + 1;
				end
			end
			return n;
		end
		local function tier(pm)
			if pm.natural then
				return 0;
			end
			if pm.placeFirst then
				return 1;
			end
			return 10;
		end
		while countReq() > 2 do
			...
			for _, pm in ipairs(pmeta) do
				if policy.coastal_required[pm.pid] == true then
					local t = tier(pm);
					if t > dropTier or (t == dropTier and (drop == nil or pm.pid > drop.pid)) then
						drop = pm;
						dropTier = t;
					end
				end
			...
			policy.coastal_required[drop.pid] = nil;
		end
		while countReq() < 2 do
			local add = nil;
			local addTier = 999;
			for _, pm in ipairs(pmeta) do
				if policy.coastal_required[pm.pid] ~= true then
					local t = tier(pm);
					if t < addTier or (t == addTier and (add == nil or pm.pid < add.pid)) then
						add = pm;
						addTier = t;
					end
				end
			...
			policy.coastal_required[add.pid] = true;
		end
	end
	return policy;
end
...
function AssignStartingPlots:LekGlobalSix_Section5_BuildPlayerBiasPlist()
	local policy = self._lek_g6_bias_policy;
	if type(policy) ~= "table" then
		policy = AssignStartingPlots.LekGlobalSix_BuildEffectiveBiasPolicy(self);
		self._lek_g6_bias_policy = policy;
	end
	local plist = {};
	for loop = 1, 6 do
		...
		if self._lek_ignore_all_start_bias == true then
			plist[#plist + 1] = { tag = "flex", pid = playerNum };
		else
		local bNeedsCoastalStart = (policy.coastal_required and policy.coastal_required[playerNum] == true);
...
function AssignStartingPlots:LekGlobalSix_TupleBiasFeasibilityFromPools(byRegion)
	...
	local enforceNC = AssignStartingPlots.LekGlobalSix_NoCoastInlandEnforced(self);
	local poolMeta = {};
	for r = 1, 6 do
		local cc, rc = false, false;
		local inl, rivInl = false, false;
		local br = byRegion[r];
		if br then
			for i = 1, #br do
				local c = br[i];
				local coastalM = (c.coastal_ok == true);
				local riverM = (c.river_ok == true);
				if c.coastal_ok == nil or c.river_ok == nil then
					local m = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, c.x, c.y);
					coastalM = (m.alongOcean == true);
					riverM = (m.isRiver == true or m.nearRiver == true);
				end
				if coastalM then
					cc = true;
				end
		...
		if not enforceNC then
			inl = true;
			rivInl = rc;
		end
		poolMeta[r] = { canCoastal = cc, canRiver = rc, canInlandPool = inl, canRiverInlandPool = rivInl };
		if cc then
		...
		if inl then
			stats.nRegionsInlandCapable = stats.nRegionsInlandCapable + 1;
		end
	end
	...
	if stats.nCoastalPlayers > stats.nRegionsCoastalCapable then
		stats.injective = 0;
		return false, "coastal_quota_pool_infeasible", stats;
	end
	...
	local function pred(p, r)
		local pm = poolMeta[r];
		if p.tag == "coastal" then
			return pm.canCoastal;
		elseif p.tag == "river" then
			if enforceNC then
				return pm.canRiverInlandPool;
			end
			return pm.canRiver;
		elseif p.tag == "pri1" then
			if enforceNC then
				return pm.canInlandPool;
			end
			return true;
		elseif p.tag == "prim" then
			...
			if enforceNC then
				return okPrim(p, r) and pm.canInlandPool;
			end
			return okPrim(p, r);
		elseif p.tag == "avoid" then
			...
			if enforceNC then
				return okAvoid(p, r) and pm.canInlandPool;
			end
			return okAvoid(p, r);
		end
		if enforceNC then
...
function AssignStartingPlots:LekGlobalSix_OK_Section5_BalanceAndAssignFeasible()
	...
	local enforceNC = AssignStartingPlots.LekGlobalSix_NoCoastInlandEnforced(self);
	local function okNonCoastalLand(r)
		return not okCoastal(r);
	end
	local nCoastalPlayers = 0;
	for _, p in ipairs(plist) do
		if p.tag == "coastal" then
			nCoastalPlayers = nCoastalPlayers + 1;
		end
	end
	local nCoastalRegions = 0;
	for r = 1, 6 do
		if okCoastal(r) then
			nCoastalRegions = nCoastalRegions + 1;
		end
	end
	if nCoastalPlayers > nCoastalRegions then
		return false, "coastal_quota_assignment_infeasible players="
			.. tostring(nCoastalPlayers) .. " regions=" .. tostring(nCoastalRegions);
	end
	local function pred(p, r)
		if p.tag == "coastal" then
			return okCoastal(r);
		elseif p.tag == "river" then
			if enforceNC then
				return okRiver(r) and okNonCoastalLand(r);
			end
			return okRiver(r);
		elseif p.tag == "pri1" then
			if enforceNC then
				return okNonCoastalLand(r);
			end
			return true;
		elseif p.tag == "prim" then
			...
			if enforceNC then
				return okPrim(p, r) and okNonCoastalLand(r);
			end
			return okPrim(p, r);
		elseif p.tag == "avoid" then
			...
			if enforceNC then
				return okAvoid(p, r) and okNonCoastalLand(r);
			end
			return okAvoid(p, r);
		end
		if enforceNC then
...
function AssignStartingPlots:LekGlobalSix_OK_RunAll()
	...
	local ok4, det4 = true, "disabled_pre_solver_coastal_quota_gate";
...
function AssignStartingPlots:LekGlobalSix_OK_LogDiagnostics()
	...
	LekPlacementProbeLog("### LekGlobalSix_OK diag runId=" .. rid ..
		...
		" s2_secondNearest_le" .. tostring(s2cap) .. mpStr .. "=" .. (R.s2_ok and "pass" or "fail") .. " " .. tostring(R.s2_det) ..
		" s3_inland_salt_dLe3_max4=" .. (R.s3_ok and "pass" or "fail") .. " " .. tostring(R.s3_det) ..
		" s4_twoCoastal_geoCycle_mesh=disabled " .. tostring(R.s4_det) ..
...
function AssignStartingPlots:LekGlobalSix_LogRuleAuditAfterTupleCommit()
	if self.iNumCivs ~= 6 then
		return;
	end
	local rid = tostring(_lek_run_id or "na");
	...
	for i = 1, 6 do
		local ti = self.startingPlots[i];
		if not ti or type(ti[1]) ~= "number" or type(ti[2]) ~= "number" then
			LekPlacementProbeLog("### LekGlobalSix ruleAuditAfterTuple runId=" .. rid .. " status=incomplete_starts", 2);
			return;
		end
		for j = i + 1, 6 do
			local tj = self.startingPlots[j];
			...
			local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, ti[1], ti[2], tj[1], tj[2]);
			if d == nil then
				LekPlacementProbeLog("### LekGlobalSix ruleAuditAfterTuple runId=" .. rid .. " status=no_PlotDistance", 2);
				return;
			end
	...
	local pairwiseOk = true;
	if type(dReq) == "number" and dReq >= 2 then
		for i = 1, 6 do
			local ti = self.startingPlots[i];
			if not ti or type(ti[1]) ~= "number" or type(ti[2]) ~= "number" then
				pairwiseOk = false;
			...
			for j = i + 1, 6 do
				...
				local req = AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, ti[1], ti[2], tj[1], tj[2], dReq);
				if d < req then
					pairwiseOk = false;
					break;
				end
			end
			if not pairwiseOk then
				break;
			end
		end
	end
	local nCoastR = 0;
	for r = 1, 6 do
		local m = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsNoNormalize(self, r);
		local isC = (m.alongOcean == true);
		if isC then
			nCoastR = nCoastR + 1;
		end
	end
...
function AssignStartingPlots:LekGlobalSix_LogRuleAuditAfterPlayers()
	if self.iNumCivs ~= 6 then
		return;
	end
	local rid = tostring(_lek_run_id or "na");
	local dsb = LekSafeGetDisableStartBiasOption();
	if dsb == 1 then
		LekPlacementProbeLog("### LekGlobalSix ruleAuditAfterPlayers runId=" .. rid .. " status=skipped_GAMEOPTION_DISABLE_START_BIAS", 2);
		return;
	end
	local plist, perr = AssignStartingPlots.LekGlobalSix_Section5_BuildPlayerBiasPlist(self);
	if not plist then
		LekPlacementProbeLog("### LekGlobalSix ruleAuditAfterPlayers runId=" .. rid .. " status=plist_err_" .. tostring(perr), 2);
		return;
	end
	local enforceNC = AssignStartingPlots.LekGlobalSix_NoCoastInlandEnforced(self);
	local avoidHard = (self._lek_global_six_s5_avoid_hard ~= false);
	local primHard = (self._lek_global_six_s5_prim_hard ~= false);
	local function okCoastalMeta(m)
		return m.alongOcean == true;
	end
	local function okRiverMeta(m)
		return (m.isRiver or m.nearRiver) == true;
	end
	local function okPrimMeta(p, r)
		if not p.pr then
			return false;
		end
		for _, rtNeed in ipairs(p.pr) do
	...
	local function okAvoidMeta(p, r)
		if not p.av then
			return true;
		end
		for _, rtBad in ipairs(p.av) do
	...
	local function regionIndexForStartXY(x, y)
		for r = 1, 6 do
			local t = self.startingPlots[r];
			if t and t[1] == x and t[2] == y then
				return r;
			end
		end
		return nil;
	...
	for i = 1, 6 do
		for j = i + 1, 6 do
			local pi = Players[self.player_ID_list[i]];
			local pj = Players[self.player_ID_list[j]];
			if not pi or not pj then
			else
				local spi, spj = pi:GetStartingPlot(), pj:GetStartingPlot();
				if spi and spj then
					local x1, y1 = spi:GetX(), spi:GetY();
					local x2, y2 = spj:GetX(), spj:GetY();
					local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, x1, y1, x2, y2);
					if d and (minD == nil or d < minD) then
						minD = d;
					end
					if type(dReq) == "number" and dReq >= 2 and d then
						local req = AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, x1, y1, x2, y2, dReq);
						if d < req then
							pairwiseOk = false;
						end
					end
	...
	for _, p in ipairs(plist) do
		...
		if not sp then
			viol = viol + 1;
			parts[#parts + 1] = "pid" .. tostring(p.pid) .. "=no_plot";
		else
			local x, y = sp:GetX(), sp:GetY();
			local m = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, x, y);
			local reg = regionIndexForStartXY(x, y);
			local bad = false;
			local why = {};
			if p.tag == "coastal" then
				if not okCoastalMeta(m) then
					bad = true;
					why[#why + 1] = "need_coastal";
				end
			elseif p.tag == "river" then
				if not okRiverMeta(m) then
					bad = true;
					why[#why + 1] = "need_river";
				end
				if enforceNC and okCoastalMeta(m) then
					bad = true;
					why[#why + 1] = "noCoastInland_violation";
				end
			else
				if enforceNC and okCoastalMeta(m) then
					bad = true;
				...
				if p.tag == "prim" and primHard and reg and not okPrimMeta(p, reg) then
					bad = true;
					why[#why + 1] = "prim_hard";
				end
				if p.tag == "avoid" and avoidHard and reg and not okAvoidMeta(p, reg) then
					bad = true;
					why[#why + 1] = "avoid_hard";
				end
			end
			if bad then
				viol = viol + 1;
			end
			local w = (#why > 0) and table.concat(why, "+") or "ok";
			parts[#parts + 1] = string.format("pid%d=%s:%s:c%s", p.pid, p.tag, w, okCoastalMeta(m) and "1" or "0");
		end
	end
	LekPlacementProbeLog("### LekGlobalSix ruleAuditAfterPlayers runId=" .. rid ..
...
function AssignStartingPlots:LekGlobalSix_LogTuplePoolDiagLine(region_r, searchOrderedN)
	...
	local rt = self.regionTypes[region_r];
	LekPlacementProbeAt(3, "### LekGlobalSix tuplePoolDiag runId=" .. rid ..
...
function AssignStartingPlots:LekGlobalSix_LogTuplePoolSpatial(region_r, pool)
	if not pool or #pool < 1 then
		return;
	end
	local rid = tostring(_lek_run_id or "na");
	local n = #pool;
	local xmin, xmax, ymin, ymax = pool[1].x, pool[1].x, pool[1].y, pool[1].y;
	local xyParts = {};
	for i = 1, n do
		local c = pool[i];
		...
		xyParts[#xyParts + 1] = tostring(c.x) .. "," .. tostring(c.y);
	end
	local bboxW = xmax - xmin + 1;
	local bboxH = ymax - ymin + 1;
	local pairSum, pairCnt = 0, 0;
	local pairMin, pairMax = nil, nil;
	local nnSum, nnCnt = 0, 0;
	if n >= 2 then
		for i = 1, n - 1 do
			for j = i + 1, n do
				local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, pool[i].x, pool[i].y, pool[j].x, pool[j].y);
				if type(d) == "number" then
					pairCnt = pairCnt + 1;
					pairSum = pairSum + d;
					if pairMin == nil or d < pairMin then pairMin = d; end
					if pairMax == nil or d > pairMax then pairMax = d; end
				end
			end
		end
		for i = 1, n do
			...
			for j = 1, n do
				if i ~= j then
					local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, pool[i].x, pool[i].y, pool[j].x, pool[j].y);
					if type(d) == "number" and (best == nil or d < best) then
						best = d;
					end
				end
			end
	...
	local pairMean = (pairCnt > 0) and (pairSum / pairCnt) or nil;
	local nnMean = (nnCnt > 0) and (nnSum / nnCnt) or nil;
	local pm = pairMean and string.format("%.2f", pairMean) or "na";
	local pmi = pairMin and tostring(pairMin) or "na";
	local pma = pairMax and tostring(pairMax) or "na";
	local nnm = nnMean and string.format("%.2f", nnMean) or "na";
	LekPlacementProbeAt(3, "### LekGlobalSix tuplePoolSpatial runId=" .. rid ..
		" region=" .. tostring(region_r) ..
		" n=" .. tostring(n) ..
		" bboxMinMax=" .. tostring(xmin) .. "," .. tostring(ymin) .. ".." .. tostring(xmax) .. "," .. tostring(ymax) ..
		" bboxWH=" .. tostring(bboxW) .. "x" .. tostring(bboxH) ..
		" pairPlotDist_min=" .. pmi .. " mean=" .. pm .. " max=" .. pma ..
		" nnPlotDist_mean=" .. nnm ..
		" hint=low_nnMean_tight_clump_high_spread" ..
		" xy=" .. table.concat(xyParts, ";"));
end
...
function AssignStartingPlots:LekGlobalSix_RegionMinCoastalBandDeviationForTuple(byRegion, region_r)
	...
	for i = 1, #list do
		local c = list[i];
		local isCoast = (c.coastal_ok == true);
		if c.coastal_ok == nil then
			local m = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, c.x, c.y);
...
function AssignStartingPlots.LekGlobalSix_DfsFeasMarginTiebreak(self, a, b)
	local m = self._lek_g6_feas_margin_by_region;
	if type(m) ~= "table" or self._lek_global_six_tuple_dfs_feas_margin == false then
		return nil;
	end
	local ma = m[a];
	local mb = m[b];
	if type(ma) ~= "number" or type(mb) ~= "number" then
		return nil;
	end
	if ma ~= mb then
		return ma < mb;
...
function AssignStartingPlots:LekGlobalSix_BuildTupleDepthOrder(byRegion)
	...
	if thinFirst and constraintWeighted then
		...
		table.sort(depthOrder, function(a, b)
			...
			local ft = AssignStartingPlots.LekGlobalSix_DfsFeasMarginTiebreak(self, a, b);
			if ft ~= nil then
				return ft;
			end
			return a < b;
	...
	if type(self._lek_g6_feas_margin_by_region) == "table" and self._lek_global_six_tuple_dfs_feas_margin ~= false
		and mode ~= "fixed_r1_r6"
	then
		mode = tostring(mode) .. "+feasMarg";
	end
	local keyParts = {};
	for ri = 1, 6 do
...
function AssignStartingPlots:LekGlobalSix_LogTupleHeadGeometry(byRegion)
	...
	for r = 1, 6 do
		...
		if not c then
			LekPlacementProbeAt(1, "### LekGlobalSix tupleHeadGeom runId=" .. rid .. " error=missing_head r=" .. tostring(r));
			return;
		end
		heads[r] = { x = c.x, y = c.y };
	...
	LekPlacementProbeAt(3, "### LekGlobalSix tupleHeadCells runId=" .. rid .. " " .. table.concat(seg, "; "));
	local pw = {};
	for i = 1, 6 do
		for j = i + 1, 6 do
	...
	LekPlacementProbeAt(3, "### LekGlobalSix tupleHeadPairwise runId=" .. rid .. " " .. table.concat(pw, " "));
	local snParts = {};
	for ri = 1, 6 do
		local dists = {};
		for j = 1, 6 do
	...
	local s2cap = self._lek_g6_runtime_s2_max or LEK_G6_S2_SECOND_NEAREST_MAX;
	LekPlacementProbeAt(3, "### LekGlobalSix tupleHeadSecondNearest runId=" .. rid ..
...
function AssignStartingPlots:LekGlobalSix_TupleSingletonFastPathTry(byRegion)
	local prod = 1;
	for r = 1, 6 do
		local n = #(byRegion[r] or {});
		if n < 1 then
	...
	for r = 1, 6 do
		local c = byRegion[r][1];
		self.startingPlots[r] = { c.x, c.y, c.score };
	...
	if R.all_hard_pass then
		tb = AssignStartingPlots.LekGlobalSix_TiebreakMaxCentrDeviation(self);
		pack = AssignStartingPlots.LekGlobalSix_CopyStartsTableForBest(self);
	end
	AssignStartingPlots.LekGlobalSix_RestorePlaceImpactState(self, baseline);
	for r = 1, 6 do
		self.startingPlots[r] = nil;
	end
	if R.all_hard_pass and pack ~= nil then
		return "ok", pack, tb, nil;
	end
...
function AssignStartingPlots:LekGlobalSix_TuplePoolFeasProbeRun(byRegion, s2cap, phaseIdx, phaseName, rid)
	...
	if type(s2cap) ~= "number" then
		s2cap = self._lek_g6_runtime_s2_max or LEK_G6_S2_SECOND_NEAREST_MAX;
	end
	local kTop = tonumber(self._lek_global_six_feas_probe_topk);
	if type(kTop) ~= "number" or kTop < 1 then
		kTop = 4;
	end
	local dReq = AssignStartingPlots.LekGlobalSix_MinPairwiseDistanceFromStartDistanceOption();
	if type(dReq) ~= "number" or dReq < 2 then
		dReq = LEK_G6_MIN_PAIRWISE_PLOTDIST_FALLBACK;
	end

	local crossCache = {};
	local function minCrossSlices(r1, r2)
		if r1 > r2 then
			r1, r2 = r2, r1;
		end
		local ck = crossCache[r1];
		if ck and ck[r2] ~= nil then
			return ck[r2];
		end
		local p1 = byRegion[r1];
		local p2 = byRegion[r2];
		if not p1 or not p2 then
			return nil;
		end
		local n1 = math.min(kTop, #p1);
		local n2 = math.min(kTop, #p2);
		...
		for i = 1, n1 do
			for j = 1, n2 do
				local a = p1[i];
				local b = p2[j];
				local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, a.x, a.y, b.x, b.y);
				if type(d) == "number" and d < best then
					best = d;
				end
			end
		end
		crossCache[r1] = crossCache[r1] or {};
		crossCache[r1][r2] = best;
		return best;
	end

	local poolNs = {};
	local minPool, maxPool = 999999, -1;
	local thinRs = {};
	for r = 1, 6 do
		local n = #(byRegion[r] or {});
		poolNs[r] = n;
	...
	local heads = {};
	for r = 1, 6 do
		local c = byRegion[r] and byRegion[r][1];
		if not c then
			_lek_bench_feas_bn = "no_head";
			_lek_bench_feas_xmin = "na";
			_lek_bench_feas_h2max = "na";
			_lek_bench_feas_margmin = "na";
			_lek_bench_feas_k = tostring(kTop);
			LekPlacementProbeLog("### LekGlobalSix tuplePoolFeasProbe runId=" .. rid ..
				" phaseIndex=" .. tostring(phaseIdx) ..
				" name=" .. tostring(phaseName or "?") ..
				" status=abort_missing_head");
			return;
		end
		heads[r] = c;
	end

	local headPairMin = 999999;
	for i = 1, 6 do
		for j = i + 1, 6 do
			local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, heads[i].x, heads[i].y, heads[j].x, heads[j].y);
			if type(d) == "number" and d < headPairMin then
				headPairMin = d;
			end
		end
	end

	local headMinD2, headMaxD2 = 999999, -1;
	local headViolR, headViolD2 = nil, nil;
	for ri = 1, 6 do
		local dists = {};
		for j = 1, 6 do
			if j ~= ri then
		...
		local d2 = dists[2];
		if d2 ~= nil then
			if d2 < headMinD2 then
				headMinD2 = d2;
			end
			if d2 > headMaxD2 then
				headMaxD2 = d2;
			end
			if d2 > s2cap and headViolR == nil then
				headViolR, headViolD2 = ri, d2;
			end
		end
	end
	if headMinD2 > 900000 then
		headMinD2 = -1;
	end
	if headMaxD2 < 0 then
		headMaxD2 = -1;
	...
	local crossGlobal = nil;
	local weakI, weakJ = nil, nil;
	for i = 1, 6 do
		for j = i + 1, 6 do
			local mc = minCrossSlices(i, j);
			if type(mc) == "number" then
				if crossGlobal == nil or mc < crossGlobal then
					crossGlobal = mc;
					weakI, weakJ = i, j;
				end
			end
	...
	local marginByR = {};
	local margMin = 999999;
	for r = 1, 6 do
		local worst = 999999;
		for j = 1, 6 do
			if j ~= r then
				local mc = minCrossSlices(r, j);
				if type(mc) == "number" and mc < worst then
					worst = mc;
				end
			end
		end
		marginByR[r] = worst;
		if worst < margMin then
			margMin = worst;
		end
	end
	self._lek_g6_feas_margin_by_region = marginByR;

	local bottleneck = "none";
	local bnNote = "";
	if #thinRs > 0 then
		bottleneck = "thin_pool";
		local tp = {};
		for _, x in ipairs(thinRs) do
			tp[#tp + 1] = tostring(x);
		end
		bnNote = "one_cell_regions=" .. table.concat(tp, ",");
	elseif headViolR ~= nil then
		bottleneck = "s2_heads";
		bnNote = "violator_r=" .. tostring(headViolR) .. " d2=" .. tostring(headViolD2) .. ">s2max=" .. tostring(s2cap);
	elseif crossGlobal ~= nil and crossGlobal < dReq then
		bottleneck = "minpair_topK";
		bnNote = "weakPair=" .. tostring(weakI) .. "," .. tostring(weakJ) ..
			" crossTopK=" .. tostring(crossGlobal) .. "<minPairReq=" .. tostring(dReq);
	end

	local pn = {};
	for r = 1, 6 do
		pn[#pn + 1] = "r" .. tostring(r) .. "=" .. tostring(poolNs[r]);
	end
	local mg = {};
	for r = 1, 6 do
		mg[#mg + 1] = "r" .. tostring(r) .. "=" .. tostring(marginByR[r]);
	end

	_lek_bench_feas_bn = bottleneck;
	_lek_bench_feas_xmin = (crossGlobal ~= nil) and tostring(crossGlobal) or "na";
	_lek_bench_feas_h2max = (headMaxD2 >= 0) and tostring(headMaxD2) or "na";
	_lek_bench_feas_margmin = (margMin < 900000) and tostring(margMin) or "na";
	_lek_bench_feas_k = tostring(kTop);
	...
	LekPlacementProbeLog("### LekGlobalSix tuplePoolFeasProbe runId=" .. rid ..
		...
		" name=" .. tostring(phaseName or "?") ..
		" kTop=" .. tostring(kTop) ..
		" minPairReq=" .. tostring(dReq) ..
		" s2max=" .. tostring(s2cap) ..
		" poolMin=" .. tostring(minPool) ..
		" poolMax=" .. tostring(maxPool) ..
		" pools{" .. table.concat(pn, ";") .. "}" ..
		" headPairMin=" .. tostring(headPairMin) ..
		" headSecondD2{min=" .. tostring(headMinD2) .. ",max=" .. tostring(headMaxD2) .. "}" ..
		" crossMinTopK=" .. ((crossGlobal ~= nil) and tostring(crossGlobal) or "na") ..
		" weakPair=" .. ((weakI ~= nil) and (tostring(weakI) .. "," .. tostring(weakJ)) or "na") ..
		" margMin=" .. tostring(margMin) ..
		" margins{" .. table.concat(mg, ";") .. "}" ..
		" bottleneck=" .. bottleneck ..
		" " .. bnNote ..
		" note=headGeometry_is_rank1_only_crossMinTopK_is_min_over_regionPairs_bestInTopK_slices_dfsTiebreak_uses_margins");
end
...
function AssignStartingPlots:LekGlobalSix_FallbackSearchCandidateForRegion(region_r)
	...
	for idx = 1, #list do
		...
		if d then
			...
			local passDisk = AssignStartingPlots.LekGlobalSix_CoastalCandidatePassesSaltWaterDiskPct(self, plotIndex);
			local rank = ringDev * 1000000 - score;
			if okSalt and passDisk
				and (best == nil or rank < bestRank or (rank == bestRank and plotIndex < best.plotIndex)) then
				bestRank = rank;
				best = {
					plotIndex = plotIndex,
					x = x,
...
function AssignStartingPlots:LekGlobalSix_HexGoalGuidanceActive()
	local g = self._lek_global_six_hex_goal_guidance;
	if g == false then
		return false;
	end
	if g == true then
	...
	return self._lek_global_six_one_map_placement_mode == true;
end

function AssignStartingPlots:LekGlobalSix_WalkHexSpoke(cx, cy, dir, R, iW, iH, wrapX, wrapY)
	local x, y = cx, cy;
	if type(GetHexNeighbor) ~= "function" or type(R) ~= "number" or R < 1 then
		return x, y;
	end
	for _ = 1, R do
...
function AssignStartingPlots:LekGlobalSix_RegionIndexAtPlotXY(x, y, iW, iH)
	...
	for r = 1, 6 do
		...
		if type(rd) == "table"
			and type(rd[1]) == "number" and type(rd[2]) == "number"
			and type(rd[3]) == "number" and type(rd[4]) == "number"
			and rd[3] >= 1 and rd[4] >= 1
		then
			if self:LekPlotXYInRegionRectangle(x, y, rd[1], rd[2], rd[3], rd[4], iW, iH) then
				return r;
			end
		end
	end
...
function AssignStartingPlots:LekGlobalSix_BuildHexGoalsForRotation(cx, cy, R, rot, iW, iH, wrapX, wrapY)
	local regByDir = {};
	local posByDir = {};
	for d = 1, 6 do
		local dir = ((d - 1 + rot) % 6) + 1;
		local vx, vy = AssignStartingPlots.LekGlobalSix_WalkHexSpoke(self, cx, cy, dir, R, iW, iH, wrapX, wrapY);
		posByDir[d] = { x = vx, y = vy };
		regByDir[d] = AssignStartingPlots.LekGlobalSix_RegionIndexAtPlotXY(self, vx, vy, iW, iH);
		if regByDir[d] == nil then
			return nil;
		end
	end
	local seen = {};
	...
	for d = 1, 6 do
		local rr = regByDir[d];
		local p = posByDir[d];
		goals[rr] = { x = p.x, y = p.y };
	end
	return goals;
end

function AssignStartingPlots:LekGlobalSix_ScoreHexGoalsForPools(byRegion, goals)
	local s = 0;
	for r = 1, 6 do
		local g = goals[r];
		if not g then
		...
		for i = 1, #list do
			local c = list[i];
			local d = AssignStartingPlots.LekGlobalSix_PlotDistance(self, c.x, c.y, g.x, g.y);
			if type(d) == "number" and d < minAll then
				minAll = d;
			end
			if self.plotDataIsCoastal[c.plotIndex] == true then
				hasC = true;
				if type(d) == "number" and d < minC then
					minC = d;
				end
			end
		end
		s = s + minAll;
		local cc = AssignStartingPlots.LekGlobalSix_CoastalCountInSearchPool(self, byRegion, r);
		if cc > 0 then
			if hasC then
				s = s + 0.28 * minC;
			else
...
function AssignStartingPlots:LekGlobalSix_NudgeHexGoalsInlandByPool(byRegion, goals, iW, iH, wrapX, wrapY)
	if goals == nil or self._lek_global_six_hex_inland_goal_nudge == false then
		return;
	end
	local a = tonumber(self._lek_global_six_hex_inland_nudge_alpha);
	if type(a) ~= "number" or a < 0 or a > 1 then
		a = 0.32;
	end
	for r = 1, 6 do
		if AssignStartingPlots.LekGlobalSix_CoastalCountInSearchPool(self, byRegion, r) == 0 then
			local g = goals[r];
			local rd = self.regionData and self.regionData[r];
			if g and type(rd) == "table" and type(rd[1]) == "number" and type(rd[2]) == "number"
				and type(rd[3]) == "number" and type(rd[4]) == "number" and rd[3] >= 1 and rd[4] >= 1
			then
				local rcx = (rd[1] + math.floor((rd[3] - 1) / 2)) % iW;
				local rcy = (rd[2] + math.floor((rd[4] - 1) / 2)) % iH;
				local nx = math.floor(g.x + (rcx - g.x) * a + 0.5);
				local ny = math.floor(g.y + (rcy - g.y) * a + 0.5);
				if type(WrapCoord) == "function" then
...
function AssignStartingPlots:LekGlobalSix_ApplyHexGoalSortToRegionPool(byRegion, r, gx, gy)
	local list = byRegion[r];
	...
	table.sort(list, function(a, b)
		...
		local da = AssignStartingPlots.LekGlobalSix_PlotDistance(self, a.x, a.y, gx, gy);
		local db = AssignStartingPlots.LekGlobalSix_PlotDistance(self, b.x, b.y, gx, gy);
...
function AssignStartingPlots:LekGlobalSix_ApplyHexGoalGuidanceToPools(byRegion)
	local rid = tostring(_lek_run_id or "na");
	self._lek_g6_hex_goals = nil;
	self._lek_g6_hex_rot = nil;
	self._lek_g6_hex_ringR = nil;
	self._lek_g6_hex_pick_score = nil;
	_lek_bench_hex_ok = "0";
	_lek_bench_hex_rot = "na";
	_lek_bench_hex_ringR = "na";
	if self.iNumCivs ~= 6 then
		return;
	end
	if not AssignStartingPlots.LekGlobalSix_HexGoalGuidanceActive(self) then
		_lek_bench_hex_ok = "off";
		return;
	end
	if type(GetHexNeighbor) ~= "function" then
		LekPlacementProbeLog("### LekGlobalSix hexGoalGuidance runId=" .. rid .. " status=skip missing_GetHexNeighbor");
		_lek_bench_hex_ok = "no_api";
		return;
	end
	local iW, iH = Map.GetGridSize();
	local wrapX = Map:IsWrapX();
	...
	local cx = math.floor(iW / 2);
	local cy = math.floor(iH / 2);
	local R = tonumber(self._lek_global_six_hex_goal_radius);
	if type(R) ~= "number" or R < 1 then
		R = 13;
	end
	local bestRot, bestGoals, bestScore = nil, nil, nil;
	for rot = 0, 5 do
		local goals = AssignStartingPlots.LekGlobalSix_BuildHexGoalsForRotation(self, cx, cy, R, rot, iW, iH, wrapX, wrapY);
		if goals then
			local sc = AssignStartingPlots.LekGlobalSix_ScoreHexGoalsForPools(self, byRegion, goals);
			if type(sc) == "number" and (bestScore == nil or sc < bestScore) then
				bestScore = sc;
				bestRot = rot;
				bestGoals = goals;
			end
	...
	if not bestGoals then
		LekPlacementProbeLog("### LekGlobalSix hexGoalGuidance runId=" .. rid ..
			" status=no_injective_rotation" ..
			" cx=" .. tostring(cx) .. " cy=" .. tostring(cy) ..
			" ringR=" .. tostring(R));
		_lek_bench_hex_ok = "0";
		return;
	end
	AssignStartingPlots.LekGlobalSix_NudgeHexGoalsInlandByPool(self, byRegion, bestGoals, iW, iH, wrapX, wrapY);
	self._lek_g6_hex_goals = bestGoals;
	self._lek_g6_hex_rot = bestRot;
	self._lek_g6_hex_ringR = R;
	self._lek_g6_hex_pick_score = bestScore;
	for r = 1, 6 do
		local g = bestGoals[r];
		if g then
			AssignStartingPlots.LekGlobalSix_ApplyHexGoalSortToRegionPool(self, byRegion, r, g.x, g.y);
		end
	end
	_lek_bench_hex_ok = "1";
	_lek_bench_hex_rot = tostring(bestRot);
	_lek_bench_hex_ringR = tostring(R);
	LekPlacementProbeLog("### LekGlobalSix hexGoalGuidance runId=" .. rid ..
		" status=applied" ..
		" rot=" .. tostring(bestRot) ..
		" ringR=" .. tostring(R) ..
		" pickScore=" .. tostring(bestScore));
end
...
function AssignStartingPlots:LekGlobalSix_GatherSearchCandidatesOrdered(region_r)
	...
	for idx = 1, #list do
		...
		elseif d < bandLo or d > bandHi then
			...
			if meetsMin then
				local okSalt, _ = AssignStartingPlots.LekGlobalSix_OK_InlandSaltCell(self, plotIndex, nil);
				local passDisk = AssignStartingPlots.LekGlobalSix_CoastalCandidatePassesSaltWaterDiskPct(self, plotIndex);
				local acc = self._lek_diag_salt_disk_accum;
				if acc and okSalt and self.plotDataIsCoastal[plotIndex] == true then
					acc.coastalMeetsInlandSalt = (acc.coastalMeetsInlandSalt or 0) + 1;
					if not passDisk then
						acc.rejects = (acc.rejects or 0) + 1;
						acc.byR[region_r] = (acc.byR[region_r] or 0) + 1;
					end
				end
				if okSalt and passDisk then
					local ringDev = math.abs(d - LEK_G6_S1_TARGET_D);
					local m = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, x, y);
					out[#out + 1] = {
						plotIndex = plotIndex,
						...
						ringDev = ringDev,
						dMapCenter = d,
						coastal_ok = (m.alongOcean == true),
						river_ok = ((m.isRiver == true) or (m.nearRiver == true)),
					};
				end
			end
		end
	...
	if #out == 0 and self._lek_global_six_tuple_empty_pool_fallback ~= false then
		...
		if fb then
			local fbm = AssignStartingPlots.LekGlobalSix_MeasureBiasConditionsAtXY(self, fb.x, fb.y);
			local rid = tostring(_lek_run_id or "na");
			LekPlacementProbeAt(3, "### LekGlobalSix tuplePoolFallback runId=" .. rid ..
				" region=" .. tostring(region_r) ..
			...
			out[1] = {
				...
				coastal_ok = (fbm.alongOcean == true),
				river_ok = ((fbm.isRiver == true) or (fbm.nearRiver == true)),
			};
		end
	end
	local cap = self._lek_global_six_max_candidates_per_region;
...
function AssignStartingPlots:LekGlobalSix_RunTupleSearch()
	...
	self._lek_tuple_cause_gate = nil;
	...
	local relaxMinLayout = self._lek_global_six_tuple_relax_min_layout;
	local forceHardOnly = false;
	local layoutN = tonumber(_lek_map_layout_attempt);
	if type(layoutN) ~= "number" or layoutN < 1 then
		layoutN = 1;
	end
	if relaxMinLayout ~= false then
		if relaxMinLayout == nil then
			relaxMinLayout = 4;
		end
		if type(relaxMinLayout) == "number" and layoutN < relaxMinLayout then
			forceHardOnly = true;
		end
	end
	...
	if forceHardOnly and type(phases) == "table" and #phases > 1 then
		phases = { phases[1] };
	end
	...
	if forceHardOnly then
		LekPlacementProbeLog("### LekGlobalSix tupleRelaxGate hard_only runId=" .. rid ..
			...
			" relax_min_layout=" .. tostring(relaxMinLayout));
	end

	local doTuplePoolDiag3 = (self._lek_tuple_pool_diag ~= false) and LekMapgenLogAtLeast(3);
	if LekMapgenLogAtLeast(2) then
		AssignStartingPlots.LekGlobalSix_LogTupleProbeLayerSnapshot(self);
	end

	local lastFailKind = nil;
	local lastFC, lastLE, lastMaxF = 0, 0, 0;
	local prevPhaseFailCombo = nil;
	local prevPhaseFailHist = nil;
	local minS2Fallback = nil;
	local minS2MaxLayout = self._lek_global_six_tuple_minimal_s2_fallback_max_layout;
	if minS2MaxLayout == nil then
		minS2MaxLayout = 3;
	end
	local minS2FallbackActive = (minS2MaxLayout ~= false) and (layoutN <= minS2MaxLayout);
	local phaseBase = phases[1];
	...
	for phaseIdx, ph in ipairs(phases) do
		...
		local skipForS5Heavy = (self._lek_global_six_tuple_skip_s2_relax_when_s5_heavy_failcombo ~= false)
			and phaseIdx > 1
			and prevPhaseFailCombo ~= nil
			and AssignStartingPlots.LekGlobalSix_TupleFailComboS5Heavy(prevPhaseFailCombo)
			and AssignStartingPlots.LekGlobalSix_TuplePhaseIsS2OnlyRelaxVsBase(ph, phaseBase);
		local pairSkipCfg = self._lek_global_six_tuple_skip_relax_when_pair_heavy;
		if pairSkipCfg == nil then
			pairSkipCfg = self._lek_global_six_tuple_skip_relax_when_s4_heavy;
		end
		local skipForPairHeavy = (pairSkipCfg ~= false)
			and phaseIdx > 1
			and prevPhaseFailHist ~= nil
			and AssignStartingPlots.LekGlobalSix_TupleFailHistPairHeavy(prevPhaseFailHist);
		if skipForPairHeavy then
			LekPlacementProbeLog("### LekGlobalSix tuplePhase skip runId=" .. rid ..
				" phaseIndex=" .. tostring(phaseIdx) .. "/" .. tostring(#phases) ..
				" name=" .. phaseName ..
				" reason=pair_heavy_failHist_minPair_hard_constraint");
			lastMaxF = maxFail;
			lastFC = 0;
			lastLE = 0;
			lastFailKind = "pair";
			self._lek_tuple_cause_prune = "pair_skip_relax";
		end
		if skipForS5Heavy then
			LekPlacementProbeLog("### LekGlobalSix tuplePhase skip runId=" .. rid ..
				" phaseIndex=" .. tostring(phaseIdx) .. "/" .. tostring(#phases) ..
				" name=" .. phaseName ..
				" reason=s5_heavy_failCombo_skip_s2_only_relax");
			lastMaxF = maxFail;
			lastFC = 0;
			lastLE = 0;
			lastFailKind = "s5";
			self._lek_tuple_cause_prune = "s5_skip_relax";
		end
		if not skipForS5Heavy and not skipForPairHeavy then
		self._lek_g6_feas_margin_by_region = nil;
		local enforceNCPhase = AssignStartingPlots.LekGlobalSix_NoCoastInlandEnforced(self);
		local nCoastalRequired = 0;
		do
			local plistPhase, _ = AssignStartingPlots.LekGlobalSix_Section5_BuildPlayerBiasPlist(self);
			if plistPhase then
				for _, p in ipairs(plistPhase) do
					if p.tag == "coastal" then
						nCoastalRequired = nCoastalRequired + 1;
					end
				end
			end
		end
		local domainStaticPruned = 0;
		local domainPrunedInlandCoastalPick = 0;
		local domainBeforeN = 0;
		local domainAfterN = 0;
		local function candCoastalOK(c)
			if c and c.coastal_ok ~= nil then
				return c.coastal_ok == true;
			end
		...
		if LekMapgenLogAtLeast(2) and AssignStartingPlots.LekGlobalSix_CoastalSaltDiskPctFilterActive(self)
			...
			self._lek_diag_salt_disk_accum = { rejects = 0, byR = {}, coastalMeetsInlandSalt = 0 };
		else
			self._lek_diag_salt_disk_accum = nil;
		end
		...
		for r = 1, 6 do
			...
			domainBeforeN = domainBeforeN + #byRegion[r];
			if enforceNCPhase and nCoastalRequired == 0 and #byRegion[r] > 0 then
				local keep = {};
				for i = 1, #byRegion[r] do
					local c = byRegion[r][i];
					if not candCoastalOK(c) then
						keep[#keep + 1] = c;
					else
						domainStaticPruned = domainStaticPruned + 1;
					end
				end
				byRegion[r] = keep;
			end
			domainAfterN = domainAfterN + #byRegion[r];
			if doTuplePoolDiag3 then
				AssignStartingPlots.LekGlobalSix_LogTuplePoolDiagLine(self, r, #byRegion[r]);
				AssignStartingPlots.LekGlobalSix_LogTuplePoolSpatial(self, r, byRegion[r]);
			end
		...
		if self._lek_diag_salt_disk_accum and LekMapgenLogAtLeast(2) then
			local acc = self._lek_diag_salt_disk_accum;
			local rad = self._lek_global_six_coastal_salt_water_disk_radius;
			if type(rad) ~= "number" or rad < 1 then
				rad = 3;
			...
			for r = 1, 6 do
				local n = #(byRegion[r] or {});
				local rej = acc.byR[r] or 0;
				sumPool = sumPool + n;
				sumVirt = sumVirt + n + rej;
				parts[#parts + 1] = "r" .. tostring(r) .. ":pool=" .. tostring(n) .. "+diskOff=" .. tostring(rej);
			end
			LekPlacementProbeLog("### LekGlobalSix coastalSaltDiskPoolDiag runId=" .. rid ..
				" phase=" .. phaseName ..
				" saltDisk_pctMax=" .. tostring(pctMax) .. " hexR=" .. tostring(rad) ..
				" coastal_meetsInlandSalt_evaluated=" .. tostring(acc.coastalMeetsInlandSalt or 0) ..
				" rejected_from_pool_by_disk=" .. tostring(acc.rejects or 0) ..
				" poolCells_total=" .. tostring(sumPool) ..
				" virtual_pool_total_if_disk_disabled=" .. tostring(sumVirt) ..
				" " .. table.concat(parts, ";"));
			self._lek_diag_salt_disk_accum = nil;
		end
		if LekMapgenLogAtLeast(2) then
			LekPlacementProbeLog("### LekGlobalSix tupleDomain phase runId=" .. rid ..
				" phaseIndex=" .. tostring(phaseIdx) ..
				" name=" .. phaseName ..
				" nCoastalRequired=" .. tostring(nCoastalRequired) ..
				" noCoastInlandEnforced=" .. (enforceNCPhase and "1" or "0") ..
				" poolBefore=" .. tostring(domainBeforeN) ..
				" poolAfter=" .. tostring(domainAfterN) ..
				" staticPruned=" .. tostring(domainStaticPruned));
		end
		...
		if abortRegion then
			...
			self._lek_tuple_cause_gate = "empty_pool_r" .. tostring(abortRegion);
			LekPlacementProbeLog("### LekGlobalSix tuplePhase end runId=" .. rid ..
				" phaseIndex=" .. tostring(phaseIdx) ..
				" name=" .. phaseName ..
				" status=skip_empty_pool");
		else
			AssignStartingPlots.LekGlobalSix_ApplyHexGoalGuidanceToPools(self, byRegion);
			local _okFeas, _errFeas = pcall(function()
				AssignStartingPlots.LekGlobalSix_TuplePoolFeasProbeRun(self, byRegion, s2m, phaseIdx, phaseName, rid);
			end);
			if not _okFeas then
				self._lek_g6_feas_margin_by_region = nil;
				_lek_bench_feas_bn = "probe_err";
				_lek_bench_feas_xmin = "na";
				_lek_bench_feas_h2max = "na";
				_lek_bench_feas_margmin = "na";
				_lek_bench_feas_k = "na";
				LekPlacementProbeLog("### LekGlobalSix tuplePoolFeasProbe pcall_fail runId=" .. rid ..
					" phaseIndex=" .. tostring(phaseIdx) ..
					" name=" .. tostring(phaseName or "?") ..
					" err=" .. tostring(_errFeas));
			end
			if doTuplePoolDiag3 then
			...
			if self._lek_global_six_tuple_bias_feasibility_gate ~= false then
				...
				LekPlacementProbeLog("### LekGlobalSix tupleBiasFeasibility runId=" .. rid ..
					...
					" regionsInlandPool=" .. tostring(st.nRegionsInlandCapable or -1) ..
					" noCoastInlandEnforced=" .. (AssignStartingPlots.LekGlobalSix_NoCoastInlandEnforced(self) and "1" or "0") ..
			...
			if not biasGateOk then
				...
				lastLE = 0;
				lastMaxF = maxFail;
				if tostring(biasGateReason) == "coastal_quota_pool_infeasible" then
					lastFailKind = "coastal_quota";
				else
					lastFailKind = "s5";
				end
				self._lek_tuple_cause_gate = tostring(biasGateReason or "bias_gate");
				LekPlacementProbeLog("### LekGlobalSix tuplePhase end runId=" .. rid ..
					" phaseIndex=" .. tostring(phaseIdx) ..
					" name=" .. phaseName ..
					" status=skip_pre_solver_gate" ..
					" detail=" .. tostring(biasGateReason));
			else
			local singletonSkipDfs = false;
			if self._lek_global_six_tuple_singleton_fastpath ~= false then
				local ss, spack, stb, sfk = AssignStartingPlots.LekGlobalSix_TupleSingletonFastPathTry(self, byRegion);
				if ss == "ok" and spack ~= nil then
					for r = 1, 6 do
						local p = spack[r];
						self.startingPlots[r] = { p[1], p[2], p[3] };
						self:PlaceImpactAndRipples(p[1], p[2]);
					end
					local _okRuleAudS, _errRuleAudS = pcall(function()
						AssignStartingPlots.LekGlobalSix_LogRuleAuditAfterTupleCommit(self);
					end);
					if not _okRuleAudS then
						LekPlacementProbeAt(1, "### LekGlobalSix ruleAuditAfterTuple pcall_err runId=" .. rid
							.. " err=" .. tostring(_errRuleAudS));
					end
					self._lek_tuple_search_success_phase_name = phaseName;
					self._lek_tuple_search_success_phase_index = phaseIdx;
					LekPlacementProbeLog("### LekGlobalSix tuplePhase success_singleton_fastpath runId=" .. rid ..
						" phaseIndex=" .. tostring(phaseIdx) ..
						" name=" .. phaseName ..
						" failComplete=0 leafEvals=1" ..
						" tiebreak_maxAbsDminus13=" .. tostring(stb));
					self._lek_g6_runtime_s1_min = nil;
					self._lek_g6_runtime_s1_max = nil;
					self._lek_g6_runtime_s2_max = nil;
					return true, 0, 1, maxFail, nil;
				elseif ss == "fail" then
					singletonSkipDfs = true;
					lastFC = 0;
					lastLE = 1;
					lastMaxF = maxFail;
					lastFailKind = sfk;
					LekPlacementProbeLog("### LekGlobalSix tuplePhase end runId=" .. rid ..
						" phaseIndex=" .. tostring(phaseIdx) ..
						" name=" .. phaseName ..
						" status=skip_dfs_singleton_tuple_precheck" ..
						" first_fail=" .. tostring(sfk) ..
						" failComplete=0 leafEvals=1" ..
						" note=prod_pool_sizes_eq_1_only_one_global_assignment_no_dfs_tree");
				end
			end

			if not singletonSkipDfs then
			...
			if self._lek_global_six_tuple_skip_dfs_rank1_head_s2 == true then
				...
				if vr0 ~= nil then
					...
					LekPlacementProbeLog("### LekGlobalSix tuplePhase skip_dfs_precheck_rank1_heads_s2 runId=" .. rid ..
						...
						" s2max=" .. tostring(s2m) ..
						" note=false_negative_non_head_tuple_can_fix_s2_set_skip_flag_false_to_disable");
				end
			end
			...
			if skipDfsRank1S2 then
				...
				lastMaxF = maxFail;
				lastFailKind = "s2";
				self._lek_tuple_cause_prune = "rank1_head_s2_precheck";
				LekPlacementProbeLog("### LekGlobalSix tuplePhase end runId=" .. rid ..
					" phaseIndex=" .. tostring(phaseIdx) ..
					" name=" .. phaseName ..
					" status=skip_dfs_rank1_head_s2_precheck failComplete=0 leafEvals=0");
			else
			local function tupleFmtFailHist(hh)
				local keys = {};
				for k, _ in pairs(hh) do
					keys[#keys + 1] = k;
				end
				table.sort(keys);
				local parts = {};
				for _, k in ipairs(keys) do
					parts[#parts + 1] = k .. ":" .. tostring(hh[k]);
				end
				return table.concat(parts, ",");
			end
			local function tupleFmtAnyHist(h)
				local keys = {};
			...
			local function tupleFmtComboHistTop(h, mkeys)
				local m = mkeys or 12;
				local arr = {};
			...
			local function tuplePhaseBudgetHint(fc, le, fh, emf, eml)
				local hitFailCap = (fc >= emf) and 1 or 0;
				local hitLeafCap = (le >= eml) and 1 or 0;
				local domK, domN = nil, -1;
				for k, n in pairs(fh) do
					if type(n) == "number" and n > domN then
						domN = n;
						domK = k;
					end
				...
				local almost = 0;
				if (hitFailCap == 1 or hitLeafCap == 1) and fc > 50 and domK ~= nil then
					almost = 1;
				end
				return hitFailCap, hitLeafCap, domK or "none", almost, hint;
			end
			local budgetRound = 0;
			local failComplete, leafEvals, bestPack, bestTb, phaseLastFail;
			local failHist, failAnyHist, failComboHist;
			local _effMaxFail, _effMaxLeaf = maxFail, maxLeaf;
			local spacingDepthCalls = 0;
			local spacingNontrivial = 0;
			repeat
				budgetRound = budgetRound + 1;
				local maxFailEff = maxFail;
				local maxLeafEff = maxLeaf;
				if budgetRound == 2 then
					maxFailEff = maxFail * 2;
					maxLeafEff = maxLeaf * 2;
					LekPlacementProbeLog("### LekGlobalSix tuplePhase budget_extension_s2_pure runId=" .. rid ..
						" phaseIndex=" .. tostring(phaseIdx) ..
						" name=" .. phaseName ..
						" maxFail=" .. tostring(maxFailEff) ..
						" maxLeaf=" .. tostring(maxLeafEff));
				end
				_effMaxFail = maxFailEff;
				_effMaxLeaf = maxLeafEff;
				failComplete = 0;
				leafEvals = 0;
				bestPack = nil;
				bestTb = nil;
				...
				failAnyHist = {};
				failComboHist = {};
				local minPairPruned = 0;
				local minPairForwardPruned = 0;
				...
				local function dfs(depth)
					if leafEvals >= maxLeafEff then
						return;
					end
					if bestPack == nil and failComplete >= maxFailEff then
						return;
					end
				if depth > 6 then
					...
					if R.all_hard_pass then
						...
						if minS2FallbackActive and R.first_fail == "s2" then
							local okm, meta = AssignStartingPlots.LekGlobalSix_QualifiesMinimalS2Plus1NearMiss(self, s2m);
							if okm then
								local tb = AssignStartingPlots.LekGlobalSix_TiebreakMaxCentrDeviation(self);
								local pack = AssignStartingPlots.LekGlobalSix_CopyStartsTableForBest(self);
								if pack then
									local better = minS2Fallback == nil or tb < minS2Fallback.tb
										or (tb == minS2Fallback.tb and Map.Rand(2, "LekMinS2FB") == 1);
									if better then
										minS2Fallback = {
											pack = pack,
											tb = tb,
						...
						if LekMapgenLogAtLeast(3) and progressEvery > 0 and failComplete % progressEvery == 0 then
							local extra = "";
							if doCompositeFailHist then
								extra = " failAnyHist=" .. tupleFmtAnyHist(failAnyHist);
							end
							LekPlacementProbeAt(3, "### LekGlobalSix tupleSearchProgress runId=" .. rid ..
								" phaseIndex=" .. tostring(phaseIdx) ..
								" name=" .. phaseName ..
								...
								" failHist=" .. tupleFmtFailHist(failHist) .. extra);
						end
					end
					return;
				end
				...
				if self._lek_global_six_tuple_spacing_reorder ~= false and depth > 1 and #cands > 1 then
					...
					if nontrivial then
						...
						if doTuplePoolDiag3 and not spacingSampleDone then
							...
							LekPlacementProbeAt(3, "### LekGlobalSix tupleSpacingReorder sample runId=" .. rid ..
								" phaseIndex=" .. tostring(phaseIdx) ..
								" name=" .. phaseName ..
				...
				local function tryCand(ci)
					...
					local c = cands[ci];
					if enforceNCPhase and candCoastalOK(c) then
						domainPrunedInlandCoastalPick = domainPrunedInlandCoastalPick + 1;
						failComplete = failComplete + 1;
						phaseLastFail = "s5";
						bumpFailHist("s5");
						return;
					...
					if dMinPairOpt6 ~= nil and depth > 1 then
						...
						for dp = 1, depth - 1 do
							...
							if t and type(t[1]) == "number" and type(t[2]) == "number" then
								local dxy = AssignStartingPlots.LekGlobalSix_PlotDistance(self, c.x, c.y, t[1], t[2]);
								local req = AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, c.x, c.y, t[1], t[2], dMinPairOpt6);
								if dxy ~= nil and dxy < req then
									hardFail = true;
									break;
								end
							end
					...
					-- Forward-check: after assigning this candidate, every unassigned region must retain
					-- at least one candidate satisfying the hard min-pair distance against assigned picks.
					if dMinPairOpt6 ~= nil and depth < 6 then
						local infeasible = false;
						for dpF = depth + 1, 6 do
							local rF = depthOrder[dpF];
							local listF = byRegion[rF];
							local hasAny = false;
							for ciF = 1, #listF do
								local cf = listF[ciF];
								local okF = true;
								for dpA = 1, depth do
									local rA = depthOrder[dpA];
									local tA = self.startingPlots[rA];
									if tA and type(tA[1]) == "number" and type(tA[2]) == "number" then
										local dxy = AssignStartingPlots.LekGlobalSix_PlotDistance(self, cf.x, cf.y, tA[1], tA[2]);
										local req = AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, cf.x, cf.y, tA[1], tA[2], dMinPairOpt6);
										if dxy ~= nil and dxy < req then
											okF = false;
											break;
										end
						...
						if infeasible then
							minPairForwardPruned = minPairForwardPruned + 1;
							failComplete = failComplete + 1;
							phaseLastFail = "pair";
							bumpFailHist("pair");
							self.startingPlots[reg] = nil;
			...
			until bestPack ~= nil or budgetRound >= 2 or not (
				self._lek_global_six_tuple_s2_pure_budget_extension ~= false
					and budgetRound == 1
					and AssignStartingPlots.LekGlobalSix_TupleShouldExtendBudgetPureS2(failHist, failComboHist)
			);

			lastFC = failComplete;
			lastLE = leafEvals;
			lastMaxF = _effMaxFail;
			lastFailKind = phaseLastFail;

			local doLogComposite = (self._lek_global_six_tuple_ok_composite_fail_hist ~= false);
			local compositeExtra = "";
			if doLogComposite and failComplete > 0 then
				compositeExtra =
					" failAnyHist=" .. tupleFmtAnyHist(failAnyHist) ..
					" failComboHist_top=" .. tupleFmtComboHistTop(failComboHist, 16);
			end
			if doLogComposite and (next(failComboHist) ~= nil or next(failAnyHist) ~= nil) then
				LekPlacementProbeLog("### LekGlobalSix tupleFailCombo runId=" .. rid ..
					" phaseIndex=" .. tostring(phaseIdx) ..
					" name=" .. phaseName ..
					" failComplete=" .. tostring(failComplete) ..
					" leafEvals=" .. tostring(leafEvals) ..
					" comboHist_top=" .. tupleFmtComboHistTop(failComboHist, 16) ..
					" anyHist=" .. tupleFmtAnyHist(failAnyHist));
			end
			...
			compositeExtra = compositeExtra ..
				" domainStaticPruned=" .. tostring(domainStaticPruned) ..
				" domainPrunedInlandCoastPick=" .. tostring(domainPrunedInlandCoastalPick);
			if dMinPairOpt6 ~= nil then
				compositeExtra = compositeExtra ..
					" minPairOpt6=" .. tostring(dMinPairOpt6) ..
					" minPairPruned=" .. tostring(minPairPruned) ..
					" minPairForwardPruned=" .. tostring(minPairForwardPruned);
			end
			if not bestPack then
				local hF, hL, domK, almostW, hintS = tuplePhaseBudgetHint(failComplete, leafEvals, failHist, _effMaxFail, _effMaxLeaf);
				self._lek_tuple_cause_leaf = tostring(phaseLastFail or domK or "na");
				self._lek_tuple_cause_combo = tupleFmtComboHistTop(failComboHist, 1);
				LekPlacementProbeLog("### LekGlobalSix tuplePhase end runId=" .. rid ..
					" phaseIndex=" .. tostring(phaseIdx) ..
					" name=" .. phaseName ..
					" status=fail_budget" ..
					" failComplete=" .. tostring(failComplete) ..
					" leafEvals=" .. tostring(leafEvals) ..
					" maxFail=" .. tostring(_effMaxFail) ..
					" maxLeaf=" .. tostring(_effMaxLeaf) ..
					...
					" hint=" .. hintS ..
					" failHist=" .. tupleFmtFailHist(failHist) .. compositeExtra);
				do
					local pc = {};
					for k, v in pairs(failComboHist) do
						if type(v) == "number" then
					...
					prevPhaseFailHist = phh;
				end
			else
				prevPhaseFailCombo = nil;
				prevPhaseFailHist = nil;
				for r = 1, 6 do
					local p = bestPack[r];
					self.startingPlots[r] = { p[1], p[2], p[3] };
					self:PlaceImpactAndRipples(p[1], p[2]);
				end
				local _okRuleAud, _errRuleAud = pcall(function()
					AssignStartingPlots.LekGlobalSix_LogRuleAuditAfterTupleCommit(self);
				end);
				if not _okRuleAud then
					LekPlacementProbeAt(1, "### LekGlobalSix ruleAuditAfterTuple pcall_err runId=" .. rid
						.. " err=" .. tostring(_errRuleAud));
				end
				self._lek_tuple_search_success_phase_name = phaseName;
				self._lek_tuple_search_success_phase_index = phaseIdx;
				self._lek_tuple_cause_leaf = "ok";
				...
				self._lek_g6_runtime_s1_max = nil;
				self._lek_g6_runtime_s2_max = nil;
				return true, failComplete, leafEvals, _effMaxFail, nil;
			end
			end
			end
			end
	...
	if minS2FallbackActive and minS2Fallback ~= nil and minS2Fallback.pack ~= nil then
		local fb = minS2Fallback;
		for r = 1, 6 do
			local p = fb.pack[r];
			self.startingPlots[r] = { p[1], p[2], p[3] };
			self:PlaceImpactAndRipples(p[1], p[2]);
		end
		local _okRuleAud2, _errRuleAud2 = pcall(function()
			AssignStartingPlots.LekGlobalSix_LogRuleAuditAfterTupleCommit(self);
		end);
		if not _okRuleAud2 then
			LekPlacementProbeAt(1, "### LekGlobalSix ruleAuditAfterTuple pcall_err runId=" .. rid
				.. " err=" .. tostring(_errRuleAud2));
		end
		self._lek_tuple_search_success_phase_name = tostring(fb.phaseName) .. "_minimal_s2_plus1";
		self._lek_tuple_search_success_phase_index = fb.phaseIdx;
		self._lek_tuple_cause_leaf = "ok_minimal_s2_plus1";
		self._lek_tuple_cause_combo = "na";
		local mm = fb.meta or {};
		LekPlacementProbeLog("### LekGlobalSix tuplePhase success_minimal_s2_plus1 runId=" .. rid ..
			" phaseIndex=" .. tostring(fb.phaseIdx) ..
			" name=" .. tostring(fb.phaseName) ..
			" layout=" .. tostring(layoutN) ..
			" nOverCap=" .. tostring(mm.nOverCap or -1) ..
		...
		self._lek_g6_runtime_s1_max = nil;
		self._lek_g6_runtime_s2_max = nil;
		return true, lastFC, lastLE, lastMaxF, nil;
	end

	self._lek_g6_runtime_s1_min = nil;
	self._lek_g6_runtime_s1_max = nil;
	self._lek_g6_runtime_s2_max = nil;
	if self._lek_tuple_cause_leaf == nil then
		self._lek_tuple_cause_leaf = tostring(lastFailKind or "na");
	end
	if self._lek_tuple_cause_combo == nil then
		self._lek_tuple_cause_combo = "na";
	end
	LekPlacementProbeAt(3, "### LekGlobalSix tupleSearch runId=" .. rid ..
...
function AssignStartingPlots:LekGlobalSixChooseLocations(args)
	...
	LekPlacementProbeAt(2, "### LekGlobalSixChooseLocations runId=" .. rid ..
	...
	LekPlacementProbeAt(1, "### g6|cause runId=" .. rid ..
		" status=" .. (okSolve and "ok" or "fail") ..
		" gate=" .. tostring(self._lek_tuple_cause_gate or "na") ..
		" prune=" .. tostring(self._lek_tuple_cause_prune or "na") ..
		" leaf=" .. tostring(self._lek_tuple_cause_leaf or "na") ..
		" combo=" .. tostring(self._lek_tuple_cause_combo or "na") ..
		" why=" .. tostring(why or "na") ..
		(okSolve and (" relax_phase=" .. tostring(self._lek_tuple_search_success_phase_name or "na")) or ""));
...
function AssignStartingPlots:ChooseLocations(args)
	...
	for _, sp in ipairs(reserve_plots) do
		local solX = (sp - 1) % iW;
		local solY = math.floor((sp - 1 - solX) / iW);
		local wrapX = Map.IsWrapX();
		local wrapY = Map.IsWrapY();
		local odd = self.firstRingYIsOdd;
		local even = self.firstRingYIsEven;
		...
		local impact_value = 99;
		self.distanceData[sp] = impact_value;
		self.playerCollisionData[sp] = true;
		for ripple_radius, ripple_value in ipairs(ripple_values) do
	...
	if self._lek_global_six_solver == true and not lekSixBiasSkipsGlobalSolver then
		...
		if self:LekGlobalSixChooseLocations(args) == true then
			...
			if _lek_enable_hb_generatemap_regen_loop == true
				...
				_lek_global_six_request_map_regen_reason = "tuple_solver_no_accepted_tuple_skip_legacy";
	...
	if self._lek_global_six_solver == true and not lekSixBiasSkipsGlobalSolver and self.iNumCivs == 6
		...
		_lek_global_six_request_map_regen_reason = "tuple_solver_no_accepted_tuple";
		_lek_global_six_request_map_regen = true;
		LekPlacementProbeAt(1, "### LekGlobalSix mapRegen request runId=" .. tostring(_lek_run_id or "na") ..
...
function AssignStartingPlots:AttemptToPlaceBonusResourceAtPlot(x, y, bAllowOasis, Fish_Count)
	...
	-- maxCapitalGoodiesLeft: per-plot branch budget for legacy bonus placements (align with iNumFoodBonusNeeded cap of 3; coastal fish re-balance uses LekPlaceFishNearStart goodies_cap=3 separately).
	--print("-"); print("Attempting to place a Bonus at: ", x, y);
	local plot = Map.GetPlot(x, y);
	local maxCapitalGoodiesLeft = 3
	...
	if plotType == PlotTypes.PLOT_OCEAN then
		if maxCapitalGoodiesLeft > 0 and maxFishPlace > 0 then
	...
	if featureType == FeatureTypes.FEATURE_JUNGLE then -- Place Banana
		if maxCapitalGoodiesLeft > 0 then
		...
		maxCapitalGoodiesLeft = maxCapitalGoodiesLeft - 1;
	...
	elseif plotType == PlotTypes.PLOT_LAND and featureType == FeatureTypes.NO_FEATURE and terrainType == TerrainTypes.TERRAIN_GRASS then
		...
		elseif placethis > 50 and placethis < 90 then
			if maxCapitalGoodiesLeft > 0 then
				plot:SetResourceType(self.sheep_ID, 1);
				AssignStartingPlots.LekMaybePromoteLandToHill(plot, 70, "Lek sheep capital_goodies grass hill pct");
				print("Placed Sheep.");
				self.amounts_of_resources_placed[self.sheep_ID + 1] = self.amounts_of_resources_placed[self.sheep_ID + 1] + 1;
				maxCapitalGoodiesLeft = maxCapitalGoodiesLeft - 1;
				return true, false, false
	...
	elseif terrainType == TerrainTypes.TERRAIN_DESERT and plotType == PlotTypes.PLOT_LAND and featureType == FeatureTypes.FEATURE_FLOOD_PLAINS then
		...
		elseif placethis > 25 and placethis < 75 then
			plot:SetResourceType(self.sheep_ID, 1);
			AssignStartingPlots.LekMaybePromoteLandToHill(plot, 100, "Lek sheep capital_goodies flood hill pct");
			...
			if maxCapitalGoodiesLeft > 0 then	-- we do actually want a limit on stone placed
				plot:SetResourceType(self.stone_ID, 1);
				print("Placed Stone.");
				self.amounts_of_resources_placed[self.stone_ID + 1] = self.amounts_of_resources_placed[self.stone_ID + 1] + 1;
				maxCapitalGoodiesLeft = maxCapitalGoodiesLeft - 1;
	...
	-- Tundra support, does not use maxCapitalGoodiesLeft (tundra path bypasses that budget for now)
...
function AssignStartingPlots:NormalizeStartLocation(region_number)
	...
	if iNumFoodBonusNeeded > 0 then
		...
		-- MOD: sapht — coastal refish / capital goodies (fish vs ring bonuses) tuning lives here
...
function AssignStartingPlots:BalanceAndAssign(args)
	...
	if _lek_pangaea_max_outer_failed == true then
	...
	if bDisableStartBias == 1 or self._lek_ignore_all_start_bias == true then
		...
		self:LekEnforceBalancedCoastalExactTwoAfterBalanceAndAssign(args);
	...
	self:LekEnforceBalancedCoastalExactTwoAfterBalanceAndAssign(args);
	if self._lek_global_six_solver == true and self.iNumCivs == 6 then
		local _okPAud, _errPAud = pcall(function()
			AssignStartingPlots.LekGlobalSix_LogRuleAuditAfterPlayers(self);
		end);
		if not _okPAud then
...
function AssignStartingPlots:AttemptToPlaceNaturalWonder(wonder_number, row_number)
	...
	if self.wonder_list[wonder_number] == "FEATURE_VOLCANO" and _krakatoa_island_plot then
	...
	elseif (((self.wonder_list[wonder_number] == "FEATURE_KING_SOLOMONS_MINES" or self.wonder_list[wonder_number] == "FEATURE_SOLOMONS_MINES") and _solomons_island_mines_plot and _solomons_island_nw_type ~= "FEATURE_GEYSER")
		...
		rest = GetShuffledCopyOfTable(rest);
		for i = 1, #rest do candidate_plot_list[i + 1] = rest[i]; end
	elseif _geothermal_island_plot and _geothermal_island_nw_type and self.wonder_list[wonder_number] == _geothermal_island_nw_type then
		local gfp = _geothermal_island_plot;
		candidate_plot_list = {gfp};
		for _, idx in ipairs(temp_table) do
			if idx ~= gfp then
				candidate_plot_list[#candidate_plot_list + 1] = idx;
			end
...
function AssignStartingPlots:PlaceNaturalWonders(wonderargs)
	...
	-- Lekmap: place forced island wonders first (Krakatoa, Solomon's Mines island, etc.) so they "steal" their slot before random process
	local forced_placed = {};
	local forced_pairs = {
...
function AssignStartingPlots:GenerateGlobalResourcePlotLists()
				...
				elseif plotType == PlotTypes.PLOT_OCEAN then
					if featureType ~= self.feature_atoll then
						...
						elseif plot:IsLake() then
							if terrainType == TerrainTypes.TERRAIN_COAST then
								table.insert(temp_lake_coast_fish, i);
							end
							self.barren_plots = self.barren_plots + 1;
						elseif terrainType == TerrainTypes.TERRAIN_COAST then
				...
				elseif plotType == PlotTypes.PLOT_HILLS and terrainType ~= TerrainTypes.TERRAIN_SNOW then
					...
					if featureType == FeatureTypes.NO_FEATURE then
						...
						elseif terrainType == TerrainTypes.TERRAIN_DESERT then
							...
							table.insert(temp_hills_open_no_grass_no_plains, i);	-- MOD.Barathor: New
						elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
							table.insert(temp_plains_hills_open, i);
							table.insert(temp_hills_open_no_tundra, i);				-- MOD.Barathor: New
							table.insert(temp_hills_open_no_desert, i);				-- MOD.Barathor: New
							table.insert(temp_hills_open_no_grass, i);				-- MOD.Barathor: New
							table.insert(temp_hills_open_no_grass_no_tundra, i);	-- MOD.Barathor: New
	...
	self.plains_hills_open_list = GetShuffledCopyOfTable(temp_plains_hills_open)
	...
	self.lake_coast_fish_list = GetShuffledCopyOfTable(temp_lake_coast_fish)
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekTryStripOneBonusFromRingsNearStart(x, y, max_ring)
	if self.fish_ID == nil or type(max_ring) ~= "number" or max_ring < 1 then
		return false;
	end
	for ring = max_ring, 1, -1 do
		for _, plotIndex in ipairs(self:LekEnumeratePlotIndicesOnHexRing(x, y, ring)) do
			...
			if plot then
				...
				if rt >= 0 and rt ~= self.fish_ID and self:LekResourceIsBonus(rt) then
...
------------------------------------------------------------------------------
-- goodies_cap: max combined count of fish + capital-goodies bonuses (LekCountCapitalGoodiesBudgetNearStart) in rings.
function AssignStartingPlots:LekPlaceFishNearStart(x, y, max_ring, count, goodies_cap)
	if count == nil or count < 1 or self.fish_ID == nil then
		return
	end
	if goodies_cap == nil then
		goodies_cap = 3;
	end
	local want = count;
	if goodies_cap > 0 then
		local maxBonus = goodies_cap - count;
		if maxBonus >= 0 then
			for _ = 1, 10 do
				local n0 = self:LekCountCapitalGoodiesBudgetNearStart(x, y, max_ring);
				if n0 <= maxBonus then
					break;
				end
				if not self:LekTryStripOneBonusFromRingsNearStart(x, y, max_ring) then
					break;
				end
			end
		end
		local n = self:LekCountCapitalGoodiesBudgetNearStart(x, y, max_ring);
		local room = math.max(0, goodies_cap - n);
		want = math.min(count, room);
		if want < count then
			if self:LekTryStripOneStoneFromRing2NearStart(x, y) then
				n = self:LekCountCapitalGoodiesBudgetNearStart(x, y, max_ring);
				room = math.max(0, goodies_cap - n);
				want = math.min(count, room);
			end
		end
	end
...
------------------------------------------------------------------------------
-- Coastal cap fish (when not refish): land lux rings 1–3 → 1 fish if nLand<4 else 2 fish; always goodies_cap=3.
-- Gate matches second-lux / sea-regional eligibility (`LekLocIsCoastalForLuxAllowAtXY`), not `IsCoastalLand()`
-- (lake-adjacent or other coastal-land tiles can differ and must not pull ocean fish when lux path is inland).
function AssignStartingPlots:LekPlaceFishForCoastalLuxPackage(x, y, region_number, secondLuxId)
	if self._lek_coastal_refish then
		return;
	end
	if not self:LekLocIsCoastalForLuxAllowAtXY(x, y) then
		return;
	end
	...
	local goodiesCap = 3;
	self:LekPlaceFishNearStart(x, y, fishRing, nFish, goodiesCap);
end
------------------------------------------------------------------------------
function AssignStartingPlots:LekSanitizeFishNearMajorLakeCells(plotDistanceLimit)
	if self.fish_ID == nil then
		return;
	end
	...
	for r = 1, self.iNumCivs or 0 do
		local sp = self.startingPlots and self.startingPlots[r];
		if sp then
			addStart(sp[1], sp[2]);
		end
	end
	for loop = 1, self.iNumCivs or 0 do
		local pid = self.player_ID_list and self.player_ID_list[loop];
		local pl = pid and Players[pid];
		local sp = pl and pl:GetStartingPlot();
		if sp then
			addStart(sp:GetX(), sp:GetY());
		end
	end
	if #starts == 0 then
		return;
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekSanitizeSeaLuxuriesOnLakeTiles()
	local iW, iH = Map.GetGridSize();
	local function collectTargets()
		...
		for y = 0, iH - 1 do
			for x = 0, iW - 1 do
				local p = Map.GetPlot(x, y);
				if p
					and p:GetPlotType() == PlotTypes.PLOT_OCEAN
					and p:GetTerrainType() == TerrainTypes.TERRAIN_COAST
					and p:IsLake() == false
					and p:GetFeatureType() ~= FeatureTypes.FEATURE_ICE
					and p:GetFeatureType() ~= self.feature_atoll
					and p:IsAdjacentToLand() == true
					and p:GetResourceType(-1) == -1
				then
					t[#t + 1] = { x = x, y = y, plot = p };
				end
			end
		end
		return t;
	...
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			...
			if p and p:IsLake() then
				local rid = p:GetResourceType(-1);
				if rid ~= -1 and self:LekIsSeaLuxuryResourceId(rid) then
					local bestI, bestD = nil, nil;
					for i = 1, #targets do
						local tg = targets[i];
						if tg.plot and tg.plot:GetResourceType(-1) == -1 then
							local d = nil;
							if Map.PlotDistance then
								d = Map.PlotDistance(x, y, tg.x, tg.y);
							elseif PlotDistance then
								d = PlotDistance(x, y, tg.x, tg.y);
							end
					...
					p:SetResourceType(-1);
					if bestI ~= nil then
						local tg = targets[bestI];
						tg.plot:SetResourceType(rid, 1);
						moved = moved + 1;
					else
						stripped = stripped + 1;
						if self.amounts_of_resources_placed and self.amounts_of_resources_placed[rid + 1] then
							self.amounts_of_resources_placed[rid + 1] = math.max(0, self.amounts_of_resources_placed[rid + 1] - 1);
						end
						if self.totalLuxPlacedSoFar and self.totalLuxPlacedSoFar > 0 then
							self.totalLuxPlacedSoFar = self.totalLuxPlacedSoFar - 1;
						end
					end
				end
	...
	if moved > 0 or stripped > 0 then
		LekMapgenFileTrace("### LekSanitizeSeaLuxuriesOnLakeTiles moved=", moved, " stripped=", stripped);
	end
end
------------------------------------------------------------------------------
function AssignStartingPlots:AssignLuxuryToRegion(region_number)
	...
	for index, resource_options in ipairs(luxury_candidates) do
		...
		if res_ID ~= nil and (self.luxury_assignment_count[res_ID] or 0) < split_cap then -- This type still eligible.
			...
			if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then -- Not a new type that would exceed number of allowed types, so continue.
				...
				elseif self:LekShouldExcludeLuxuryFromRegionalAssignment(res_ID) then
				else
					table.insert(resource_IDs, res_ID);
		...
		for index, resource_options in ipairs(self.luxury_fallback_weights) do
			...
			if res_ID ~= nil and (self.luxury_assignment_count[res_ID] or 0) < 3 then -- This type still eligible.
				...
				if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then
					if not self:LekShouldExcludeLuxuryFromRegionalAssignment(res_ID) then
						table.insert(resource_IDs, res_ID);
						local adjusted_weight = resource_options[2] / (1 + (self.luxury_assignment_count[res_ID] or 0))
						table.insert(resource_weights, adjusted_weight);
						iNumAvailableTypes = iNumAvailableTypes + 1;
	...
	if iNumAvailableTypes == 0 then
		for index, resource_options in ipairs(self.luxury_fallback_weights) do
			local res_ID = resource_options[1];
			if res_ID ~= nil and (self.luxury_assignment_count[res_ID] or 0) < 3 then
				local test = TestMembership(self.resourceIDs_assigned_to_regions, res_ID)
				if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then
					table.insert(resource_IDs, res_ID);
					local adjusted_weight = resource_options[2] / (1 + (self.luxury_assignment_count[res_ID] or 0))
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekShouldExcludeLuxuryFromRegionalAssignment(res_ID)
	if res_ID == nil then
		return true;
	end
	...
	return res_ID == self.salt_ID or res_ID == self.spices_ID or res_ID == self.gems_ID
		or res_ID == self.obsidian_ID or res_ID == self.marble_ID or res_ID == self.rubber_ID
	...
	for index, region_info in ipairs(self.regions_sorted_by_type) do
		...
		if resource_ID == nil then
			for _, ro in ipairs(self.luxury_fallback_weights) do
				local rid = ro[1];
				if rid ~= nil and not self:LekShouldExcludeLuxuryFromRegionalAssignment(rid) then
					resource_ID = rid;
					break;
				end
			end
...
function AssignStartingPlots:LekLocIsCoastalForLuxAllowAtXY(x, y)
	...
	local iW = select(1, Map.GetGridSize());
	local idx = y * iW + x + 1;
	if self.plotDataIsCoastal[idx] ~= true then
		return false;
	end
	local p = Map.GetPlot(x, y);
	if not p then
	...
	for _, d in ipairs({
		...
		local np = Map.PlotDirection(x, y, d);
		if np and np:GetPlotType() == PlotTypes.PLOT_OCEAN and np:IsLake() == false then
			return true;
		end
	end
	return false;
...
function AssignStartingPlots:LekRegionHasCoastalMajorStartForLux(region_number)
	...
	if sp and type(sp[1]) == "number" and type(sp[2]) == "number" then
		return self:LekLocIsCoastalForLuxAllowAtXY(sp[1], sp[2]) == true;
	end
	return false;
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:GenerateLuxuryPlotListsInRegion(region_number)
	...
	if iAreaID ~= -1 then
		local okArea, ao = pcall(function()
			return Map.GetArea(iAreaID);
		end);
		if okArea then
			region_area_object = ao;
		end
	end
					...
					if plot:IsLake() == false then
						if featureType ~= self.feature_atoll and featureType ~= FeatureTypes.FEATURE_ICE then
							if iAreaID == -1 then
								...
								local useCoast = false;
								if region_area_object ~= nil then
									local okAdj, vAdj = pcall(function()
										return plot:IsAdjacentToArea(region_area_object);
									end);
									if okAdj and vAdj == true then
										useCoast = true;
									end
								end
								if useCoast then
									table.insert(region_coast, plotIndex);
								elseif region_area_object == nil and plot:IsAdjacentToLand() then
									table.insert(region_coast, plotIndex);
								end
							end
						end
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekEnumeratePlotIndicesOnHexRing(cx, cy, ring)
	local out = {};
	...
	local even = self.firstRingYIsEven;
	local nextX, nextY, plot_adjustments;
	local currentX = cx - ring;
	local currentY = cy;
	for direction_index = 1, 6 do
		for plot_to_handle = 1, ring do
			if currentY / 2 > math.floor(currentY / 2) then
				plot_adjustments = odd[direction_index];
			else
				plot_adjustments = even[direction_index];
			...
			elseif wrapY == false and (nextY < 0 or nextY >= iH) then
				...
				table.insert(out, realY * iW + realX + 1);
			end
			currentX, currentY = nextX, nextY;
		end
	end
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekMarblePerfumeCoerceHillAtPlot(x, y)
	local plot = Map.GetPlot(x, y);
	if plot == nil then
		return false;
	end
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekEnsureHillForMarblePerfumePlot(x, y)
	local plot = Map.GetPlot(x, y);
	if plot == nil then
		return false;
	end
	...
	if pt == PlotTypes.PLOT_MOUNTAIN or pt == PlotTypes.PLOT_OCEAN then
		return false;
	end
	if self:AttemptToPlaceHillsAtPlot(x, y) then
		return true;
	end
	return self:LekMarblePerfumeCoerceHillAtPlot(x, y);
end
------------------------------------------------------------------------------
function AssignStartingPlots:LekBuildMarblePerfumeStartPlotListRing3Hill(luxury_plot_lists, sx, sy, pi, ps, pt, pq, pqi, pse)
	local iW = select(1, Map.GetGridSize());
	local unionKeys = {};
	local function addList(li)
		if type(li) == "number" and li > 0 and luxury_plot_lists[li] then
			for _, plotIndex in ipairs(luxury_plot_lists[li]) do
				unionKeys[plotIndex] = true;
			end
		end
	end
	addList(pi);
	...
	local coerced = {};
	local seen = {};
	local function consider(plotIndex)
		if seen[plotIndex] then
			return;
		end
		local px = math.floor((plotIndex - 1) % iW);
		local py = math.floor((plotIndex - px - 1) / iW);
		local d = plotDist(sx, sy, px, py);
		if d ~= 3 then
			return;
		end
		...
		local pt = plot:GetPlotType();
		if pt == PlotTypes.PLOT_OCEAN or pt == PlotTypes.PLOT_MOUNTAIN then
			return;
		end
		if pt == PlotTypes.PLOT_HILLS then
			table.insert(naturals, plotIndex);
			seen[plotIndex] = true;
			return;
		end
		if pt == PlotTypes.PLOT_LAND then
			if self:LekEnsureHillForMarblePerfumePlot(px, py) then
				local p2 = Map.GetPlot(px, py);
				if p2 and p2:GetPlotType() == PlotTypes.PLOT_HILLS and p2:GetResourceType(-1) == -1 then
					table.insert(coerced, plotIndex);
					seen[plotIndex] = true;
				end
			end
		end
	end
	for plotIndex, _ in pairs(unionKeys) do
		consider(plotIndex);
	end
	for _, plotIndex in ipairs(self:LekEnumeratePlotIndicesOnHexRing(sx, sy, 3)) do
		if not seen[plotIndex] then
			consider(plotIndex);
		end
	end
	local out = GetShuffledCopyOfTable(naturals);
	for _, plotIndex in ipairs(GetShuffledCopyOfTable(coerced)) do
		table.insert(out, plotIndex);
	end
	return out;
end
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	elseif resource_ID == self.ivory_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 11, 16, 8, 39, 22, 33;
...
function AssignStartingPlots:LekBuildRegionalLuxuryRepairPoolInBand(region_number, res_ID, dmin, dmax)
	...
	if self:LekIsSeaLuxuryResourceId(res_ID) then
		local raw = self:LekCollectCoastSeaPlotIndicesInBandFromXY(sx, sy, dmin, dmax);
		local out = {};
		local dropLand, dropCS = 0, 0;
		for _, plotIndex in ipairs(raw) do
			if not AssignStartingPlots.LekCoastalRegionalTilePassesWorkableLandRule(self, plotIndex) then
				dropLand = dropLand + 1;
			elseif AssignStartingPlots.LekIsPlotWithinDistanceOfAnyCityState(self, plotIndex, 2) then
				dropCS = dropCS + 1;
			else
				out[#out + 1] = plotIndex;
			end
		end
		if dropLand > 0 or dropCS > 0 then
			LekMapgenFileTrace("### LEK SEA REGIONAL REPAIR FILTER runId=", tostring(_lek_run_id or "na"),
				" Region#", region_number, " raw=", #raw, " keep=", #out,
				" dropLandR2=", dropLand, " dropNearCS=", dropCS, " band=", tostring(dmin) .. "-" .. tostring(dmax));
		end
		return out;
	end
	local iW = select(1, Map.GetGridSize());
...
function AssignStartingPlots:LekAssertRegionalLuxuryNoShortfallAfterFallbackOrRegen()
	...
	if canRegen then
		_lek_global_six_request_map_regen_reason = "regional_lux_shortfall_post_fallback";
		_lek_global_six_request_map_regen = true;
		return;
	end
	...
	for rn = 1, self.iNumCivs do
		...
		if sp and type(sp[1]) == "number" and type(sp[2]) == "number" then
			...
			if nLux < minLux then
				...
				LekMapgenFileTrace(fs);
				if canRegen then
					_lek_global_six_request_map_regen_reason = "capital_lux_tile_min_gate";
					_lek_global_six_request_map_regen = true;
					return;
				end
				if self._lek_global_six_one_map_placement_mode == true then
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekCountMajorPlayerStartsAdjacentToSaltWater()
	local n = 0;
	local iW, iH = Map.GetGridSize();
	if not self.plotDataIsCoastal then
		return 0;
	end
	for loop = 1, self.iNumCivs or 0 do
		local pid = self.player_ID_list[loop];
...
------------------------------------------------------------------------------
-- After BalanceAndAssign: exactly min(2,iNumCivs) majors on plotDataIsCoastal tiles (same as Force2 assert).
-- If too many: swap salt-adjacent majors off coast, prioritizing demoting coastal-bias civs.
-- If too few: FindCoastalStart on non-salt regions (prefer regions held by non-coastal-bias civs).
function AssignStartingPlots:LekEnforceBalancedCoastalExactTwoAfterBalanceAndAssign(args)
	if self.BalancedCoastalExactTwo ~= true or self.ForceAllInlandPlayerSpawns == true then
		return;
	end
	if (self.iNumCivs or 0) < 2 or not self.plotDataIsCoastal then
		return;
	end
	args = args or {};
	local iW, iH = Map.GetGridSize();
	...
	local function plotIsSalt(x, y)
		if type(x) ~= "number" or type(y) ~= "number" or x < 0 or x >= iW or y < 0 or y >= iH then
			return false;
		end
		return self.plotDataIsCoastal[y * iW + x + 1] == true;
	end

	local function civNeedsCoastalPlayer(pid)
		local pl = LekSafeMajorPlayer(pid);
		if not pl then
			return false;
		end
		local row = GameInfo.Civilizations[pl:GetCivilizationType()];
		if not row then
			return false;
		end
		local civType = row.Type;
		local b = CivNeedsCoastalStart(civType);
		if args.MixedBias and Map.Rand(100, "") >= 0 and CivNeedsPlaceFirstCoastalStart(civType) then
			b = false;
		end
		return b;
	...
	local function findRegionForXY(x, y)
		for r = 1, self.iNumCivs or 0 do
			local t = self.startingPlots[r];
			if t and t[1] == x and t[2] == y then
				return r;
	...
	local function pidAtPlot(x, y)
		for loop = 1, self.iNumCivs or 0 do
			local pid = self.player_ID_list[loop];
			local pl = LekSafeMajorPlayer(pid);
			if pl and pl:IsAlive() then
				local sp = pl:GetStartingPlot();
				if sp and sp:GetX() == x and sp:GetY() == y then
					return pid;
				end
			end
		end
	...
	local function swapPair(pidA, pidB)
		local plA = LekSafeMajorPlayer(pidA);
		local plB = LekSafeMajorPlayer(pidB);
		if not plA or not plB or not plA:IsAlive() or not plB:IsAlive() then
			return false;
		end
		local spA = plA:GetStartingPlot();
		local spB = plB:GetStartingPlot();
		if not spA or not spB then
			return false;
		end
		local xa, ya = spA:GetX(), spA:GetY();
		local xb, yb = spB:GetX(), spB:GetY();
		local rA = findRegionForXY(xa, ya);
		local rB = findRegionForXY(xb, yb);
		if not rA or not rB or rA == rB then
			return false;
		end
		plA:SetStartingPlot(spB);
		plB:SetStartingPlot(spA);
		local zA = (self.startingPlots[rA] and self.startingPlots[rA][3]) or 0;
		local zB = (self.startingPlots[rB] and self.startingPlots[rB][3]) or 0;
		self.startingPlots[rA] = { xb, yb, zA };
		self.startingPlots[rB] = { xa, ya, zB };
		return true;
	end

	local function countSaltPlayers()
		return self:LekCountMajorPlayerStartsAdjacentToSaltWater();
	end

	for _pass = 1, 8 do
		local n = countSaltPlayers();
		if n == needExact then
			break;
		end
		if n > needExact then
			local excess = n - needExact;
			local atSalt = {};
			local inland = {};
			for loop = 1, self.iNumCivs or 0 do
				local pid = self.player_ID_list[loop];
				...
				if pl and pl:IsAlive() then
					...
					if sp then
						local x, y = sp:GetX(), sp:GetY();
						if plotIsSalt(x, y) then
							atSalt[#atSalt + 1] = { pid = pid, need = civNeedsCoastalPlayer(pid) };
						else
							inland[#inland + 1] = { pid = pid, need = civNeedsCoastalPlayer(pid) };
						end
					end
				end
			end
			table.sort(atSalt, function(a, b)
				if a.need ~= b.need then
					return a.need;
				end
				return a.pid < b.pid;
			end);
			table.sort(inland, function(a, b)
				if a.need ~= b.need then
					return not a.need;
				end
				return a.pid < b.pid;
			...
			local swapped = 0;
			for i = 1, math.min(excess, #atSalt, #inland) do
				if swapPair(atSalt[i].pid, inland[i].pid) then
					swapped = swapped + 1;
				end
			end
			if swapped == 0 then
		...
		elseif n < needExact then
			local deficit = needExact - n;
			local regionCandidates = {};
			for r = 1, self.iNumCivs or 0 do
				local t = self.startingPlots[r];
				if t and type(t[1]) == "number" and type(t[2]) == "number" then
					if not plotIsSalt(t[1], t[2]) then
						local pid = pidAtPlot(t[1], t[2]);
						if pid then
							regionCandidates[#regionCandidates + 1] = {
								r = r,
								pid = pid,
								need = civNeedsCoastalPlayer(pid),
							};
						end
					end
				end
			end
			table.sort(regionCandidates, function(a, b)
				if a.need ~= b.need then
					return not a.need;
				end
				return a.r < b.r;
			...
			local fixed = 0;
			for i = 1, math.min(deficit, #regionCandidates) do
				local r = regionCandidates[i].r;
				local pid = regionCandidates[i].pid;
				local ok = self:FindCoastalStart(r);
				if ok then
					local nt = self.startingPlots[r];
					if nt and type(nt[1]) == "number" and type(nt[2]) == "number" then
						local np = Map.GetPlot(nt[1], nt[2]);
						local pl = LekSafeMajorPlayer(pid);
						if pl and np then
							pl:SetStartingPlot(np);
							fixed = fixed + 1;
						end
					end
				end
	...
	local nFinal = countSaltPlayers();
	if nFinal ~= needExact then
		local msg = "### LEK FORCE2 postBA runId=" .. tostring(_lek_run_id or "na")
			.. " saltPlayers=" .. tostring(nFinal) .. " need_exact=" .. tostring(needExact) .. " ###";
		print(msg);
		LekMapgenFileTrace(msg);
		local maxRegenL = _lek_global_six_regen_max_layouts;
		if type(maxRegenL) ~= "number" or maxRegenL < 1 then
			maxRegenL = 4;
		end
		...
		local canRegen = AssignStartingPlots.LekGlobalSix_CanRequestLayoutRegenForPlacementGate(self) and att < maxRegenL;
		if canRegen then
			_lek_global_six_request_map_regen_reason = "force2_coastal_post_ba";
			_lek_global_six_request_map_regen = true;
		end
	end
end
...
function AssignStartingPlots:LekAssertBalancedCoastalExactTwoOrRegen()
	...
	local att = _lek_map_layout_attempt or 1;
	local fs = "### LEK FORCE2 preBA runId=" .. tostring(_lek_run_id or "na")
		.. " saltAdjRegionStarts=" .. tostring(nSalt) .. " need_exact=" .. tostring(needExact)
		.. " layout=" .. tostring(att) .. "/" .. tostring(maxRegenL)
		.. " note=postBalance_enforces_player_salt_count ###";
	print(fs);
	LekMapgenFileTrace(fs);
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:LekAssertGlobalSixPlayerMinPairwiseOrRegen()
	...
	elseif type(dReq) == "number" and dReq >= 2 then
		local n = self.iNumCivs or 0;
		local coords = {};
		for loop = 1, n do
			local player_ID = self.player_ID_list[loop];
		...
		if #coords == n then
			for i = 1, n do
				for j = i + 1, n do
					...
					local req = AssignStartingPlots.LekGlobalSix_MinPairRequiredPlotDistForTuple(self, a[1], a[2], b[1], b[2], dReq);
					if d < req then
						bad = true;
						reason = "pair_ij=" .. tostring(i) .. "," .. tostring(j)
							.. " dist=" .. tostring(d) .. " required>=" .. tostring(req);
						break;
					end
				end
				if bad then
	...
	if canRegen then
		_lek_global_six_request_map_regen_reason = "major_min_pairwise_gate";
		_lek_global_six_request_map_regen = true;
		return;
	end
	if self._lek_global_six_one_map_placement_mode == true then
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekCountWorkableLandTilesInRadiusFromXY(x, y, max_ring)
	local n = 0;
	local function bump(plot)
		if not plot then
			return;
		...
		local pt = plot:GetPlotType();
		if (pt == PlotTypes.PLOT_LAND or pt == PlotTypes.PLOT_HILLS) and pt ~= PlotTypes.PLOT_MOUNTAIN then
			n = n + 1;
		end
	end
	bump(Map.GetPlot(x, y));
...
------------------------------------------------------------------------------
function AssignStartingPlots:LekCoastalRegionalTilePassesWorkableLandRule(plotIndex)
	if type(plotIndex) ~= "number" or plotIndex < 1 then
		return false;
	end
	local iW = select(1, Map.GetGridSize());
	local x = (plotIndex - 1) % iW;
	local y = (plotIndex - x - 1) / iW;
	return self:LekCountWorkableLandTilesInRadiusFromXY(x, y, 2) >= 4;
end
------------------------------------------------------------------------------
function AssignStartingPlots:LekIsPlotWithinDistanceOfAnyCityState(plotIndex, max_plot_distance)
	if type(plotIndex) ~= "number" or plotIndex < 1 then
		return false;
	end
	local md = max_plot_distance;
	if type(md) ~= "number" or md < 0 then
		md = 1;
	end
	local iW = select(1, Map.GetGridSize());
	local x = (plotIndex - 1) % iW;
	...
	for city_state = 1, self.iNumCityStates or 0 do
		if self.city_state_validity_table and self.city_state_validity_table[city_state] == true then
			...
			if st and type(st[1]) == "number" and type(st[2]) == "number" then
				local d = nil;
				if Map.PlotDistance then
					d = Map.PlotDistance(x, y, st[1], st[2]);
				elseif PlotDistance then
					d = PlotDistance(x, y, st[1], st[2]);
				end
				if type(d) == "number" and d <= md then
					return true;
				end
			end
		end
...
------------------------------------------------------------------------------
-- Counts fish + bonus resources (LekResourceIsBonus) on capital and in hex rings — "capital goodies" for coastal fish cap.
function AssignStartingPlots:LekCountCapitalGoodiesBudgetNearStart(x, y, max_ring)
	local n = 0;
	local pc = Map.GetPlot(x, y);
	if pc then
...
function AssignStartingPlots:LekPlaceStartingRegionalLuxBundleForRegion(region_number)
	...
	local coastalRuleChecked = 0;
	local coastalRuleDropped = 0;
	local coastalRuleDroppedCS = 0;
	local function filterCoastalRegionalRule(lst)
		if capCoast ~= true or lst == nil or #lst == 0 then
			return lst;
		end
		local out = {};
		for _, idx in ipairs(lst) do
			coastalRuleChecked = coastalRuleChecked + 1;
			if AssignStartingPlots.LekCoastalRegionalTilePassesWorkableLandRule(self, idx) then
				if AssignStartingPlots.LekIsPlotWithinDistanceOfAnyCityState(self, idx, 2) then
					coastalRuleDroppedCS = coastalRuleDroppedCS + 1;
				else
					out[#out + 1] = idx;
				end
			else
				coastalRuleDropped = coastalRuleDropped + 1;
			end
		end
		return out;
	end
	local function orderPlotsForStartLux(lst, resId)
		...
		if resId == this_region_luxury then
			lst = filterCoastalRegionalRule(lst);
			if lst == nil or #lst == 0 then
				return lst;
			end
		end
	...
	if capCoast and coastalRuleChecked > 0 then
		LekMapgenFileTrace("### LEK COASTAL REGIONAL R2_LAND_RULE runId=", tostring(_lek_run_id or "na"),
			" Region#", region_number, " checked=", coastalRuleChecked,
			" dropped=", coastalRuleDropped, " dropNearCS=", coastalRuleDroppedCS,
			" kept=", (coastalRuleChecked - coastalRuleDropped - coastalRuleDroppedCS));
	end
end
------------------------------------------------------------------------------
	...
	for attempt = 1, 2 do
		...
		for _, rn in ipairs(short) do
			...
			if not self._lek_second_lux_start_ok[rn] then
				if self:LekLocIsCoastalForLuxAllowAtXY(x, y) and not self._lek_coastal_refish then
...
function AssignStartingPlots:PlaceLuxuries()
	...
	for loop, reg_data in ipairs(self.regions_sorted_by_type) do
		...
		local coastalRuleDropped = 0;
		local coastalRuleDroppedCS = 0;
		local function orderStartRegionalPlots(lst, resId)
			if lst == nil or #lst == 0 then
				return lst;
			end
			if capCoast and resId == this_region_luxury then
				local out = {};
				for _, idx in ipairs(lst) do
					coastalRuleChecked = coastalRuleChecked + 1;
					if AssignStartingPlots.LekCoastalRegionalTilePassesWorkableLandRule(self, idx) then
		...
		if this_region_luxury == self.marble_ID or this_region_luxury == self.perfume_ID then
			luxury_plot_lists = self:GenerateLuxuryPlotListsAtCitySite(x, y, 3, false);
			local ring3hills = self:LekBuildMarblePerfumeStartPlotListRing3Hill(luxury_plot_lists, x, y, primary, secondary, tertiary, quaternary, quinary, senary);
			local shuf_list = orderStartRegionalPlots(ring3hills, this_region_luxury);
			iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumToPlace, 1, -1, 0, 0, shuf_list);
		else
			-- First pass, checking only first two rings with a 50% ratio.
			luxury_plot_lists = self:GenerateLuxuryPlotListsAtCitySite(x, y, 2, false)
			local shuf_list = orderStartRegionalPlots(luxury_plot_lists[primary], this_region_luxury)
			iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumToPlace, 0.5, -1, 0, 0, shuf_list);
			if iNumLeftToPlace > 0 and secondary > 0 then
				shuf_list = orderStartRegionalPlots(luxury_plot_lists[secondary], this_region_luxury)
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 0.5, -1, 0, 0, shuf_list);
			end
			if iNumLeftToPlace > 0 and tertiary > 0 then
				shuf_list = orderStartRegionalPlots(luxury_plot_lists[tertiary], this_region_luxury)
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 0.5, -1, 0, 0, shuf_list);
			end
			if iNumLeftToPlace > 0 and quaternary > 0 then
				shuf_list = orderStartRegionalPlots(luxury_plot_lists[quaternary], this_region_luxury)
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 0.5, -1, 0, 0, shuf_list);
			end
			if iNumLeftToPlace > 0 and quinary > 0 then
				shuf_list = orderStartRegionalPlots(luxury_plot_lists[quinary], this_region_luxury)
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 0.5, -1, 0, 0, shuf_list);
			end
			if iNumLeftToPlace > 0 and senary > 0 then
				shuf_list = orderStartRegionalPlots(luxury_plot_lists[senary], this_region_luxury)
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 0.5, -1, 0, 0, shuf_list);
			end
			...
			if iNumLeftToPlace > 0 then
				...
				luxury_plot_lists = self:GenerateLuxuryPlotListsAtCitySite(x, y, 3, false)
				shuf_list = orderStartRegionalPlots(luxury_plot_lists[primary], this_region_luxury)
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 1, -1, 0, 0, shuf_list);
				if iNumLeftToPlace > 0 and secondary > 0 then
					shuf_list = orderStartRegionalPlots(luxury_plot_lists[secondary], this_region_luxury)
					iNumLeftToPlace = self:PlaceSpecificNumberOfResources(this_region_luxury, 1, iNumLeftToPlace, 1, -1, 0, 0, shuf_list);
	...
	for region_number = 1, self.iNumCivs do
		...
		if iNumLeftToPlace > 0 then
			table.insert(self._lek_regional_lux_fallback_queue, { region_number = region_number, res_id = res_ID, amount = iNumLeftToPlace });
		end
		end
	end
	...
	if self.iNumTypesRandom > 0 then
		LekMapgenFileTrace("* *", "* iNumTypesRandom = ", self.iNumTypesRandom, "* *");
		...
		LekMapgenFileTrace("* *", "* targetLuxForThisWorldSize = ", targetLuxForThisWorldSize, "* *");
		LekMapgenFileTrace("* *", "* random to add to target = ", extraLux, "* *");
		LekMapgenFileTrace("* *", "* totalLuxPlacedSoFar = ", self.totalLuxPlacedSoFar, "* *");
		LekMapgenFileTrace("* *", "* iNumRandomLuxTarget = ", iNumRandomLuxTarget, "* *");
...
function AssignStartingPlots:PlaceLuxuries_OLD()
	...
	if self.resource_setting ~= 1 then
		for region_number = 1, self.iNumCivs do
			...
			if (not secondaryPlaced) then
				...
				LekMapgenFileTrace("Failed to place second Luxury type at start in Region#", region_number, "-- no eligible positions!");
			end
		end
	end
	...
	for region_number = 1, self.iNumCivs do
		...
		if self:LekIsSeaLuxuryResourceId(res_ID) then
			...
			filterSeaList(primary); filterSeaList(secondary); filterSeaList(tertiary); filterSeaList(quaternary);
			if seaDropLand > 0 or seaDropCS > 0 then
				LekMapgenFileTrace("### LEK SEA REGIONAL FILTER runId=", tostring(_lek_run_id or "na"),
					" Region#", region_number, " raw=", seaRaw,
					" dropLandR2=", seaDropLand, " dropNearCS=", seaDropCS,
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					if plot:GetResourceType(-1) == -1 and featureType ~= FeatureTypes.FEATURE_OASIS then
						...
						elseif use_this_ID == self.maize_ID then
							if (plotType == PlotTypes.PLOT_LAND or plotType == PlotTypes.PLOT_HILLS) and terrainType == TerrainTypes.TERRAIN_PLAINS and featureType == FeatureTypes.NO_FEATURE then
								table.insert(plot_list, plotIndex);
							end
						elseif use_this_ID == self.sheep_ID then
							if featureType == FeatureTypes.NO_FEATURE and plotType == PlotTypes.PLOT_HILLS then
...
------------------------------------------------------------------------------
function AssignStartingPlots.LekFlattenHillForOpenResource(plot, hill_flatten_percent, map_rand_key)
	if plot == nil then
		return;
	end
	local pct = hill_flatten_percent;
	...
	local pt = plot:GetPlotType();
	if pt == PlotTypes.PLOT_HILLS then
		if pct >= 100 or Map.Rand(100, map_rand_key or "Lek hill flat") < pct then
...
function AssignStartingPlots.LekMaybePromoteLandToHill(plot, promote_percent, map_rand_key)
	if plot == nil then
		return;
	end
	local pct = promote_percent;
	if type(pct) ~= "number" then
		pct = 100;
	end
	if pct < 0 then
	...
	if pct >= 100 or Map.Rand(100, map_rand_key or "Lek land hill") < pct then
		plot:SetPlotType(PlotTypes.PLOT_HILLS, false, true)
	end
end

function AssignStartingPlots.LekMergePlotIndexLists(...)
	local out = {};
	for li = 1, select("#", ...) do
		local list = select(li, ...);
		if list ~= nil then
			for i = 1, #list do
				out[#out + 1] = list[i];
			end
...
function AssignStartingPlots:LekFilterLakeFishIndicesAwayFromMajorStarts(plot_indices, min_plot_distance)
	if plot_indices == nil or #plot_indices == 0 or self.startingPlots == nil then
		return plot_indices or {};
	end
	local dlim = min_plot_distance;
	if type(dlim) ~= "number" or dlim < 0 then
		dlim = 4;
	end
	local iW, iH = Map.GetGridSize();
	local out = {};
...
function AssignStartingPlots.LekPlotEligibleForNeighborForest(p)
	if p == nil or p:IsWater() then
		return false;
	end
	local pt = p:GetPlotType();
	...
	local t = p:GetTerrainType();
	if t == TerrainTypes.TERRAIN_SNOW then
		return false;
	end
	if t == TerrainTypes.TERRAIN_DESERT and p:GetFeatureType() ~= FeatureTypes.FEATURE_FLOOD_PLAINS then
		return false;
	end
	return true;
end

function AssignStartingPlots.LekFurDeerEnsureForestAdjacency(plot, x, y)
	if plot == nil then
		return;
	end
	...
	for i = 1, #dirs do
		local np = Map.PlotDirection(x, y, dirs[i]);
		if np ~= nil then
			local nft = np:GetFeatureType();
			if nft == FeatureTypes.FEATURE_FOREST or nft == FeatureTypes.FEATURE_JUNGLE then
				return;
			end
		end
	end
	...
	for i = 1, #dirs do
		local np = Map.PlotDirection(x, y, dirs[i]);
		if np ~= nil and AssignStartingPlots.LekPlotEligibleForNeighborForest(np) then
			candidates[#candidates + 1] = np;
		end
	end
	if #candidates > 0 then
		candidates[Map.Rand(#candidates, "Lek fur deer forest neighbor") + 1]:SetFeatureType(FeatureTypes.FEATURE_FOREST, -1);
	elseif AssignStartingPlots.LekPlotEligibleForNeighborForest(plot) then
		plot:SetFeatureType(FeatureTypes.FEATURE_FOREST, -1);
	end
end
------------------------------------------------------------------------------
		...
		local terrainType = plot:GetTerrainType()
			...
			if Map.Rand(100, "") < deerForestPct then
				...
				if res_ID == self.fur_ID then
					AssignStartingPlots.LekFlattenHillForOpenResource(plot, 70, "Lek fur hill flat");
				end
				...
				local rain = LekMapGetCustomOption(2)	
				if rain == 1 then		
					-- Arid
					AvgJungleRange = 0.08
				elseif rain == 3 then	
			...
			if res_ID == self.deer_ID or res_ID == self.fur_ID then
				AssignStartingPlots.LekFurDeerEnsureForestAdjacency(plot, x, y);
			end
			...
			elseif res_ID == self.incense_ID or 
				...
				if res_ID == self.incense_ID or res_ID == self.perfume_ID or res_ID == self.wine_ID then
					AssignStartingPlots.LekFlattenHillForOpenResource(plot, 70, "Lek res open lux hill pct");
				else
					AssignStartingPlots.LekFlattenHillForOpenResource(plot, 100, "Lek res open lux hill flat");
				end
				
				if (featureType ~= FeatureTypes.FEATURE_FLOOD_PLAINS) and not (res_ID == self.ivory_ID and featureType == FeatureTypes.FEATURE_JUNGLE) then
					plot:SetFeatureType(FeatureTypes.NO_FEATURE, -1)
				end
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	local maize_plots = GetShuffledCopyOfTable(AssignStartingPlots.LekMergePlotIndexLists(self.plains_flat_no_feature, self.plains_hills_open_list));
	self:ProcessResourceList(35 * bonus_multiplier, 3, maize_plots, resources_to_place)
	
	
end
------------------------------------------------------------------------------
function AssignStartingPlots:LekTestCopperOnAllWater()
	if _lek_test_copper_on_all_water ~= true then
		return;
	end
	local iW, iH = Map.GetGridSize();
	local rid = self.copper_ID;
	if rid == nil then
		LekMapgenDiagLogAppend("### _lek_test_copper_on_all_water skip copper_ID=nil");
		return;
	end
	local attempts, matched = 0, 0;
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			local plot = Map.GetPlot(x, y);
			local pt = plot:GetPlotType();
			if pt == PlotTypes.PLOT_OCEAN or pt == PlotTypes.PLOT_LAKE then
				attempts = attempts + 1;
				pcall(function()
					plot:SetResourceType(rid, 2);
				end);
				if plot:GetResourceType(-1) == rid then
					matched = matched + 1;
				end
			end
		end
	end
	LekMapgenDiagLogAppend("### _lek_test_copper_on_all_water attempts=" .. tostring(attempts) .. " matched=" .. tostring(matched) .. " copper_ID=" .. tostring(rid));
end
...
function AssignStartingPlots:PlaceResourcesAndCityStates()
	...
	LekMapgenFileTrace("Map Generation - Placing Luxuries");
	do
	...
	do
		local ok, err = pcall(function() self:LekTestCopperOnAllWater(); end);
		if not ok then
	...
	self:LekSanitizeFishNearMajorLakeCells(4);
	self:LekSanitizeSeaLuxuriesOnLakeTiles();
	if _lek_global_six_request_map_regen ~= true then
