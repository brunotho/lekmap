...
| 2026-03-31 | **Implementation note:** **`LekGlobalSix_OK_LogDiagnostics`** — measures legacy six-tuple vs **`OK` §1–§2** only (log line **`### LekGlobalSix_OK diag`** when **`_lek_global_six_solver=true`**); §3–§6 still to wire into full **`OK()`** + search. |
