------------------------------------------------------------------------------
--	CommaIsland.lua (recreated from transcript)
...
function TryPlaceCommaIsland(plotTypes, centerX, centerY, islLandInRing, params)
	...
	if #landTiles < 5 or #landTiles > 7 then return false; end
	if not footprintClear(plotTypes, landTiles, params.iW, params.iH) then return false; end

	DrawCommaIsland(plotTypes, landTiles, bodyTiles, params.iW);
	return true;
end

function DrawCommaIsland(plotTypes, landTiles, bodyTiles, iW)
	local bodySet = {};
	for _, t in ipairs(bodyTiles) do bodySet[t[1] .. "," .. t[2]] = true; end
