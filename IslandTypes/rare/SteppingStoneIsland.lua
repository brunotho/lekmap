...
function TryPlaceSteppingStoneIsland(plotTypes, opts)
	...
	plotTypes[coastY * iW + coastX + 1] = PlotTypes.PLOT_MOUNTAIN;
	local stepType = PlotTypes.PLOT_MOUNTAIN;

	local stepSize = (Map.Rand(100, "") < 15) and 2 or 1;
	plotTypes[stepY * iW + stepX + 1] = stepType;
	if stepSize == 2 then
		...
		if sx2 >= 0 and sx2 < iW and sy2 >= 0 and sy2 < iH and plotTypes[sy2 * iW + sx2 + 1] == PlotTypes.PLOT_OCEAN then
			plotTypes[sy2 * iW + sx2 + 1] = stepType;
		end
	end
	...
	if Map.Rand(100, "") < 20 then
		...
		if farX and farY >= 0 and farY < iH and isWater(plotTypes, farX, farY, iW) then
			...
			plotTypes[farY * iW + farX + 1] = (Map.Rand(100, "") < 50) and PlotTypes.PLOT_HILLS or PlotTypes.PLOT_LAND;
			for _ = 1, farSize - 1 do
				...
				if fx >= 0 and fx < iW and fy >= 0 and fy < iH and plotTypes[fy * iW + fx + 1] == PlotTypes.PLOT_OCEAN then
					plotTypes[fy * iW + fx + 1] = (Map.Rand(100, "") < 50) and PlotTypes.PLOT_HILLS or PlotTypes.PLOT_LAND;
					farX, farY = fx, fy;
				end
			end
		end
